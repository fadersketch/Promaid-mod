...
- 建造投影区块外不可见修复（用户："玩家仍然是一走出自己所划好的红色区块里面的橙色幽灵方块从外面就看不见了"——一百八十九删距离剔除后仍未解决，本次定位到可见性层）：
  - 【日志实证·数据链完好】`projection: request → size=28x89x28 chars=256967 → received 2600 blocks`——请求/生成/下发/接收全通，PROJECTIONS 里就有 2600 个点；问题在【渲染可见性】不是数据
  - 【根因】2600 个半透明盒（alpha=0.20）从区块外看：只有朝玩家的【外皮】几个面可见（其余被内部块体自身遮挡），0.20 的透明度又极淡 → 红框（大包围盒+填充）醒目、幽灵方块几乎看不出；一百四十七"框能显示幽灵必能显示"对近景成立，对"从外看远景"不成立
  - 【修复】①幽灵面 **alpha 0.20 → 0.45**（金预览青色 0.22 → 0.40）；②玩家与区块中心**水平距离 >24 格时**，每盒补画**棱线描边**（DebugRenderer 线条——从外看建筑轮廓清晰可辨；近处密盒肉眼可辨，不画省性能）
  - 测试：重启游戏 → 建造区块 → 玩家走到红框外 30~60 格 → 橙影轮廓（描边+加浓面）应清晰可见；走近后描边停画、面显示照常

## 实测二百

- 行为运行期间平桥门槛补全（用户选句："只挡启动、行为起来后 5 格内照样铺" → "行为起来以后也挡"）：
  - 【缺口】一百九十九只给平桥腿（dy < minDy 的 tryAirBridgeStep）加了距离门槛；**斜上台阶腿（tryDiagStep，dy≥1 时朝主人方向垫前方台阶）没加**——行为启动后主人高 1~2 格、水平 <5 格时，她照样朝水平方向垫台阶（用户实测看到的就是这条腿）
  - 【修复】斜上台阶腿与启动门/平桥腿同口径：**水平距离 < startHDist 时不垫**；「垫脚下」的垂直垫高（placeStep——不朝水平方向产生方块）保留，竖直搭高逻辑不受影响
  - 测试：重启游戏 → 行为已启动（铺桥/爬台阶中），水平距离缩到 5 格内 → 不再朝水平方向垫任何方块（走路逼近；2.5 格内垂直垫柱照常）

## 实测一百九十九

- 平桥启动距离默认值调整 + 平桥腿距离门槛补全（用户："给搭路再加一个配置项目。水平距离小于 5 的时候不会触发上次改的水平搭建方块。此项目仍然可以在面板内自己进行配置"）：
  - 【现状】该配置其实一百八十七已存在（「平桥启动距离（格）」bridgeStartHDist，面板可调 3~64），默认 6；按用户要求默认改为 **5**（含面板行/配置注释/行为类注释同步）
  - 【补全·tick 平桥腿门槛】检查发现一百八十七的 startHDist 只加在 **canUse 启动门**上——行为启动后每 tick 的平桥腿（tryAirBridgeStep）没有距离门槛：一经启动就一路铺到 2.5 格，"小于 5 不触发"只挡启动、行为起来后 5 格内照样铺桥。本次给 tick 平桥腿加上同口径门槛（hDist < startHDist 时不铺，走路逼近/竖直垫高不受影响）——真正的"水平距离小于 5 不铺水平方块"
  - 测试：重启游戏 → 女仆与主人水平距 4 格隔沟 → 不铺桥（走路/跟随处理）；距 6 格 → 正常铺桥逼近；行为中途距离缩到 <5 时不再继续铺，走路贴到 2.5 格收尾；竖直搭高不受影响
