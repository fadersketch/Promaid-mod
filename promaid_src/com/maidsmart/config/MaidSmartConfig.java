...
public static final ForgeConfigSpec.BooleanValue MISC_DIMENSION_FOLLOW;
    ...
    COMBAT_HEAL_COOLDOWN = BUILDER.comment("治疗食物冷却（tick）")
        ...
        COMBAT_TETHER_ENABLE = BUILDER.comment("武装拴绳总开关（默认开）：手持武装拴绳右击【飞行中的】女仆，把自己挂到她下方（默认 2.6 格，像武装直升机的二号位枪手）——她照常飞、照常用远程武器开火，你也一样；再右击一次（或按潜跳）解除。**地面不再自动解除**（「绳子不会自己断」，实测六百七十一）；只有落水超过 0.6 秒会把你放下（溺水是致命的）；解除瞬间人在空中给 5 秒摔伤豁免；挂着时卡墙/挤墙伤全免。\n\n【规则】只认主人；一只女仆同时只挂一人；扫帚模式必须她已经飞在空中（地面挂着会把人拖进地里），空袭模式（flight_combat / flight_ranged）随时可挂；绑定后**空袭不再改她的高度**（「不要套用悬空起程」：她照常自己爬升/俯冲，绳子只负责把你吊在下面），扫帚仍是原地悬停（不再绕着你转圈），接敌照旧；挂着期间你和女仆同款免摔落/卡墙/挤墙/撞墙伤。她本人对你的伤害本来就被「主人/友军免伤总闸」拦着，挂多紧都不会被她自己打中。\n\n【二号位开火】挂着时照常射击（弓/弩/枪械都行）——她打她的目标，你打你瞄的，各自独立。合成：拴绳 + 铁锭×2。日志搜「武装拴绳」")
                .translation("config.promaid.tether.enable").define("enable", true);
        COMBAT_TETHER_HANG = BUILDER.comment("悬挂距离（格，默认 2.6，0.5~6.0）：玩家脚底到女仆脚底的垂直距离，也就是那根「不会断」的绳子的长度。默认 2.6 = 她的脚底高过你的视线（眼高约 1.62 格），前方视野让开（实测六百七十一 玩家反馈「视线会被女仆的建模挡住」）；调小 = 人贴在她身上（1.8 以下两个碰撞箱会重叠），调大 = 吊得更低（更像吊机）。定位是平滑滑变的，不会上下横跳")
