...
public final class CompressionBoxService {
    ...
    public static ItemStack carryOf(ServerPlayer player) {
        Carry c = CARRIES.get(player.m_20148_());
        return c == null ? ItemStack.f_41583_ : c.stack;
    }

    /** 上一次被拒的原因（没有 = 空串）——随内容同步发给客户端，界面上画成一行红字 */
    public static String noticeOf(ServerPlayer player) {
        String s = NOTICES.get(player.m_20148_());
        return s == null ? "" : s;
    }
    ...
    private static void notice(ServerPlayer player, String text) {
        if (text == null || text.isEmpty()) {
            NOTICES.remove(player.m_20148_());
        } else {
            NOTICES.put(player.m_20148_(), text);
        }
    }
    ...
    public static void returnCarry(ServerPlayer player) {
        Carry c = CARRIES.remove(player.m_20148_());
        if (c == null || c.stack.m_41619_()) {
            return;
        }
        ItemStack left = c.stack;
        c.stack = ItemStack.f_41583_;
        try {
            left = ItemHandlerHelper.insertItemStacked(
                    new PlayerMainInvWrapper(player.m_150109_()), left, false);
        } catch (Throwable ignored) {
            // 背包访问出问题也要掉出来，不能留在手里
        }
        if (!left.m_41619_()) {
            player.m_36176_(left, false); // drop：装不下的掉脚边
        }
    }
    ...
    public static void tickPlayer(ServerPlayer player) {
        ...
        if (c.stack.m_41619_()) {
            CARRIES.remove(player.m_20148_()); // 手上空着：顺手把空壳清掉，别一直留着
            return;
        }
        if (!CompressionBoxData.isBox(player.m_21120_(handOf(c.hand)))) {
            returnCarry(player);
        }
    }
    ...
    public static boolean handle(ServerPlayer player, int handOrdinal, int action, int index,
        ...
        try {
            ItemStack box = player.m_21120_(handOf(handOrdinal));
            ...
            IItemHandler inv = new PlayerMainInvWrapper(player.m_150109_());
            // 新的一次点击：先清掉上一次的拒绝提示（本次真被拒了会重新写上）
            notice(player, null);
            if (action <= DEPOSIT_ONE) {
                // 旧口径（自检一直在跑这几条，保留）
                boolean changed = action <= TAKE_ALL
     ...
     * {@link CompressionBoxData#write} 落盘：万一中途抛异常，盒子的 NBT 还是原样
    ...
    private static boolean give(ServerPlayer player, IItemHandler inv, List<ItemStack> work,
        ...
        if (!back.m_41619_()) {
            player.m_36176_(back, false); // drop
        }
     ...
     * 中间任何一步抛异常（写标签失败、合并不下）都会让那份东西**只离开背包、没进盒子**
    ...
    private static boolean deposit(ServerPlayer player, IItemHandler inv, ItemStack box,
        ...
        int want = Math.min(amount, from.m_41613_());
        List<ItemStack> work = CompressionBoxData.duplicate(items);
        ItemStack left = CompressionBoxData.merge(work, from.m_255036_(want));
    ...
    private static boolean clickBox(List<ItemStack> items, int slot, Carry c, int button) {
        ...
        int want = button == 1 ? 1 : c.stack.m_41613_();
        ...
        c.stack.m_41774_(moved); // shrink：手上少掉放下去的那些
        if (c.stack.m_41619_()) {
            c.stack = ItemStack.f_41583_;
        }
        return true;
    ...
    private static boolean clickInv(ServerPlayer player, IItemHandler inv, int slot, Carry c,
        ...
        if (c.stack.m_41619_()) {
            ...
            // 拿起：左键整叠，右键一半（原版口径）
            int want = button == 1 ? (cur.m_41613_() + 1) / 2 : cur.m_41613_();
        ...
        if (cur.m_41619_()) {
            return placeIntoSlot(inv, slot, c, button == 1 ? 1 : c.stack.m_41613_());
        }
        if (ItemStack.m_150942_(cur, c.stack)) {
            return placeIntoSlot(inv, slot, c, button == 1 ? 1 : c.stack.m_41613_());
        }
        if (button == 1) {
        ...
        int limit = Math.min(inv.getSlotLimit(slot), c.stack.m_41741_());
        if (c.stack.m_41613_() > limit) {
            return false; // 放不下整叠（例：手上 100 个、这一格上限 64）→ 不换，免得退不回去
        }
        ItemStack old = inv.extractItem(slot, cur.m_41613_(), false);
        if (old.m_41613_() != cur.m_41613_()) {
