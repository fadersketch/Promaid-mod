 ...
 * 分支，1.20.1 实证 = {@code RenderType.m_110467_}；它的透明态才是 TRANSLUCENT）
 * **配上顶点 alpha = 0.15**（同一个类的 {@code renderToBuffer(..., flag ? 0.15F : 1.0F)}）。
 * 本类把这两件事都做上：① 换成同贴图的 itemEntityTranslucentCull；② 把顶点色里的 alpha 乘上配置值。
 *
 * ── 怎么做到"只影响她、只影响这一个视角"──
 * 拦在 {@code EntityMaidRenderer.m_7392_} 的 HEAD（TLM 三条渲染分支——YSM / Gecko / Bedrock——
 ...
 * 于是第三人称、别的玩家、别人骑别的女仆、以及"她只是被骑着但没拴绳"的情况都不受影响。
 *
 * ── 贴图怎么来（不写死名单、也不猜名字）──
...
public final class MaidGhostRender {
    ...
    private static boolean isGhostTarget(EntityMaid maid) {
        ...
        if (mc.f_91074_.m_20202_() != maid) {
            return false; // 他没骑着她 = 没挂在下面（牵绳档不幽灵化）
        }
        return com.maidsmart.combat.GunnerTetherManager.isGunner(maid, mc.f_91074_);
    }
    ...
    public static MultiBufferSource wrap(MultiBufferSource src, EntityMaidRenderer renderer, EntityMaid maid) {
        ...
        final RenderType fallbackKey = tex == null ? null : RenderType.m_110458_(tex);
        final RenderType fallbackVal = tex == null ? null : RenderType.m_110467_(tex);
        return type -> {
            ...
            return new GhostVertex(src.m_6299_(mapped), mul);
        };
    }
    ...
    private static Object translucentRef() {
        if (translucentRef == null) {
            Object got = MISS;
            try {
                Object state = fieldValue(RenderType.m_110467_(DUMMY_TEX),
     ...
     *
     * <p>只重写 9 个 abstract 方法就够——1.20.1 的 {@code VertexConsumer} 里 {@code color(float×4)}、
     * {@code color(int)}、{@code uv2(int)}、{@code overlayCoords(int)} 这些便捷重载**都是 default 且都
     * 回落到这几个 abstract 上**（javap 实证：default 体里 invokeinterface 的就是 m_6122_ / m_7120_ /
     * m_7122_），所以模型与图层传下来的颜色一个都跑不掉。
     *
     * <p>【刻意**不**压 m_7404_（defaultColor）】它是"顶点自己没写颜色时才用的默认色"，两版约定
     * 不一致（0~255 / 0~1 各有说法），压错了会把颜色弄脏；而女仆这条链路上没有一个调用点用它
     * （Bedrock 模型走 ModelPart 逐顶点写色、Gecko 走 renderCubesOfBone 逐顶点写色）。
     */
    private static final class GhostVertex implements VertexConsumer {
        ...
        public VertexConsumer m_6122_(int r, int g, int b, int a) {
            d.m_6122_(r, g, b, Math.max(0, Math.min(255, Math.round(a * mul))));
            return this;
        }
