...
/**
 * v1.2.0（1.20.1）：飞行**远战**任务——图标 = 弓，**不响应自主切换**。
 *
 ...
 *   `EntityMaid.m_6504_(performRangedAttack)` → 因为本任务实现了 {@link IRangedAttackTask}，
 *   会回调到下面的 {@link #performRangedAttack}。
...
public class MaidFlightRangedTask implements IRangedAttackTask {
    ...
    public ItemStack getIcon() {
        return new ItemStack(ForgeRegistries.ITEMS.getValue(new ResourceLocation("minecraft", "elytra")));
    }
     ...
     *   （1.20.1 是 `TridentItem extends Item implements Vanishable`），旧版会在这里被
     *   判掉、一枪都放不出来；
     * - **御币**（TLM 的 `ItemHakureiGohei`）：走 TLM 的弹幕 API `DanmakuShoot`——
     *   与 TLM 自己的 `TaskDanmakuAttack` 同一套（反编译实证：御币的
     *   `m_6394_/shootProjectile` 是**空实现**、supportedProjectiles 恒 true，
     *   说明它压根不走原版箭矢通道，硬造箭必然变成"射箭"）；
     * - **其它弹射武器**（弓/弩/模组弹射物）：造箭，弹药优先按武器自己的判据找。
     */
    @Override
    ...
    public static void fireRangedWeapon(EntityMaid shooter, LivingEntity target, float power) {
        try {
            ItemStack weapon = shooter.m_21205_();
            ...
            if (isGohei(weapon)) {
                // 御币：走 TLM 弹幕通道（含扇形——见 shootDanmaku 的扇叶分档规则）
    ...
    /** 是否 TLM 御币（博丽/早苗两种——TLM 自带 isGohei 判据） */
    ...
    private static boolean isTrident(ItemStack stack) {
        return stack.m_41720_() instanceof net.minecraft.world.item.TridentItem;
    }

    /**
     * 三叉戟投掷——照搬 TLM `TaskTridentAttack.performRangedAttack`（1.20.1 字节码实证）。
     *
     * 为什么不能走原版那条路：`TridentItem.m_5551_(releaseUsing)` 第一件事就是
     * `if (!(entityLiving instanceof Player)) return;`（字节码 `instanceof Player; ifne`），
     * 女仆不是 Player → 调用它等于空转。所以必须自己建 `ThrownTrident`，这也正是
     * TLM 给女仆做三叉戟任务的做法。
     *
     * 与 TLM 逐点一致（不是自己发挥，避免出现"比玩家强/有刷取漏洞"的差异）：
     * ① 投的是**副本**——主手那把只掉 1 点耐久，不会被真的扔出去；
     * ② **剥掉忠诚附魔**（`Enchantments.f_44955_`，官方 mappings 实证即 LOYALTY）——
     *    留着它，`ThrownTrident.m_37594_()` 对活着的女仆返回 true，这把副本会飞回来、
     *    还可能被玩家捡走，等于凭空多出一把三叉戟；
    ...
    private static void throwTrident(EntityMaid shooter, LivingEntity target) {
        try {
            // v1.2.0 实测五百三十二【激流三叉戟投不出去】：原版 `TridentItem.m_5551_` 里
            // "造 ThrownTrident"那一支**只在激流 = 0 时走**（字节码 `iload 7; ifne` 跳过），
            // 激流 > 0 走的是旋转突进分支。TLM 的 TaskTridentAttack 不看激流（照投），
            // 本模组原本也照搬——这里补上原版闸门：带激流就不投。
            // 正常路径上她根本不会拿着激流三叉戟进远程空袭（MaidFlightKit.isRangedWeapon
            // 已把它排除、三件套不齐），这条是防任务切换竞态的兜底。
            if (MaidFlightKit.isRiptide(shooter.m_21205_())) {
                return;
            }
            ItemStack stack = shooter.m_21205_().m_41777_();
            // 剥忠诚：1.20.1 是 Map<Enchantment,Integer>，取出来删掉再写回（TLM 同款）
            try {
                java.util.Map<net.minecraft.world.item.enchantment.Enchantment, Integer> ench =
                        net.minecraft.world.item.enchantment.EnchantmentHelper.m_44831_(stack);
                net.minecraft.world.item.enchantment.Enchantment loyalty =
                        net.minecraft.world.item.enchantment.Enchantments.f_44955_;
                if (!ench.isEmpty() && ench.containsKey(loyalty)) {
                    ench.remove(loyalty);
                    net.minecraft.world.item.enchantment.EnchantmentHelper.m_44865_(ench, stack);
                }
            } catch (Throwable ignored) {
            }
            net.minecraft.world.entity.projectile.ThrownTrident trident =
                    new net.minecraft.world.entity.projectile.ThrownTrident(shooter.m_9236_(), shooter, stack);
            double dx = target.m_20185_() - shooter.m_20185_();
            double dy = target.m_20188_() - shooter.m_20188_();
            double dz = target.m_20189_() - shooter.m_20189_();
            float dist = shooter.m_20270_(target);
            float velocity = net.minecraft.util.Mth.m_14036_(dist / 10.0f, 1.6f, 3.2f);
            float inaccuracy = 1.0f - net.minecraft.util.Mth.m_14036_(dist / 100.0f, 0.0f, 0.9f);
            trident.m_20242_(true);
            trident.m_6686_(dx, dy, dz, velocity, inaccuracy);
            trident.f_36705_ = net.minecraft.world.entity.projectile.AbstractArrow.Pickup.CREATIVE_ONLY;
            shooter.m_21205_().m_41622_(1, shooter, m -> m.m_21166_(EquipmentSlot.MAINHAND));
            shooter.m_9236_().m_7967_(trident);
     ...
     *
     * @param distanceFactor 由 TLM 的开火通道传入（`MaidShootTargetTask` 的蓄力系数），
     *                       与 TLM 同款用于伤害/速度缩放
     */
    private static void shootDanmaku(EntityMaid shooter, LivingEntity target, float distanceFactor) {
        try {
            float dmg = 4.0f;
            try {
                dmg = (float) shooter.m_21133_(net.minecraft.world.entity.ai.attributes.Attributes.f_22281_);
            } catch (Throwable ignored) {
            }
            ...
            long sameKind = 0L;
            try {
                var mem = shooter.m_6274_().m_21952_(
                        net.minecraft.world.entity.ai.memory.MemoryModuleType.f_148204_);
                if (mem.isPresent()) {
            ...
            // 多重射击附魔（TLM 用的是 f_44959_ = multishot，反编译实证）
            int multiShot = 0;
            try {
                multiShot = net.minecraft.world.item.enchantment.EnchantmentHelper
                        .m_44843_(net.minecraft.world.item.enchantment.Enchantments.f_44959_,
                                shooter.m_21205_());
            } catch (Throwable ignored) {
            }
            // v1.2.0 实测四百九十二【御币三个专属附魔补齐】——TLM 的阻碍/神速/末影杀手是
            // 注册在 GOHEI 类别上的附魔（反编译 InitEnchantments：EnchantmentCategory.create(
            // "gohei", item -> item instanceof ItemHakureiGohei)），它们**自身没有行为**，
            // 作用全靠 TaskDanmakuAttack 读等级后传给弹幕 API。TLM 原文三处：
            //   speed 再乘 (speedyLevel + 1)；setImpedingLevel(impedingLevel)；
            //   setHurtEnderman(endersEnderLevel > 0)。
            // 旧版整段跳过（当时的注释写"保持与旧版一致：不吃这三个"），本次按用户
            // "攻击能不能触发附魔效果"的要求补齐，算式与 TLM 逐字一致。
            int impedingLevel = 0;
            int speedyLevel = 0;
            boolean endersEnder = false;
            try {
                impedingLevel = net.minecraft.world.item.enchantment.EnchantmentHelper
                        .m_44843_(com.github.tartaricacid.touhoulittlemaid.init.InitEnchantments
                                .IMPEDING.get(), shooter.m_21205_());
                speedyLevel = net.minecraft.world.item.enchantment.EnchantmentHelper
                        .m_44843_(com.github.tartaricacid.touhoulittlemaid.init.InitEnchantments
                                .SPEEDY.get(), shooter.m_21205_());
                endersEnder = net.minecraft.world.item.enchantment.EnchantmentHelper
                        .m_44843_(com.github.tartaricacid.touhoulittlemaid.init.InitEnchantments
                                .ENDERS_ENDER.get(), shooter.m_21205_()) > 0;
            } catch (Throwable ignored) {
            }

            float distance = shooter.m_20270_(target);
            ...
            float speed = 1.6f * (distanceFactor + 1.0f) * (float) (speedyLevel + 1)
                    + net.minecraft.util.Mth.m_14036_(distance / 40.0f - 0.4f, 0.0f, 2.4f);
            // TLM 原式：inaccuracy = 1 - clamp(dist/100, 0, 0.8)
            float inaccuracy = 1.0f - net.minecraft.util.Mth.m_14036_(distance / 100.0f, 0.0f, 0.8f);
    ...
    private static boolean enemyEntityTest(EntityMaid shooter, LivingEntity target, LivingEntity test) {
        try {
            return shooter.m_6779_(test)
                    && target.m_6095_().equals(test.m_6095_())
                    && shooter.m_142582_(test);
        } catch (Throwable ignored) {
            return false;
        }
    }
     ...
     *
     * 【根因·反编译实证（1.20.1 客户端 SRG 字节码）】原版弩的附魔**不在弓那条路里**：
     * <ul>
     *   <li>`CrossbowItem.m_40914_`(createProjectile) 给每支箭额外打三样：
     *       `m_36740_(SoundEvents.f_11840_)`（命中音 = CROSSBOW_HIT）、
     *       `m_36793_(true)`（setShotFromCrossbow）、
     *       `m_36767_((byte) 等级)`（穿透，读 `Enchantments.f_44961_` = PIERCING）；</li>
     *   <li>`CrossbowItem.m_40887_`(performShooting) 按 `Enchantments.f_44959_`(MULTISHOT)
     *       决定发数：**1 发 / 3 发**，三发的偏角在字节码里就是 **0° / -10° / +10°** 三个 ldc；</li>
     *   <li>`CrossbowItem.m_40939_`(getChargeDuration) = **25 - 5 × `f_44960_`(QUICK_CHARGE)**——
     *       快速装填作用在"蓄力时长"上；飞行时没有蓄力动作，所以由
     *       `MaidFlightCombatBehavior#rangedShotCooldown` 按同一相对幅度缩短开火间隔。</li>
     * </ul>
     *
     * 旧版把弩当弓处理（一律走 {@link #shootArrow}），于是**多重射击 / 穿透 100% 丢失**；
     * 而弩根本附不上 Power/Punch/Flame，所以"弩的附魔一个都不触发"就是这么来的。
     *
     * 【弹药只吃 1 份】原版 `loadProjectiles` 对 Multishot 的第 2、3 发用的是副本，
     * 只有第 1 发真的从背包扣——所以"1 支箭变 3 支"正是这个附魔的价值，这里照抄。
    ...
    private static void shootCrossbowArrows(EntityMaid shooter, LivingEntity target, ItemStack crossbow,
        ...
        ItemStack ammo = findArrow(shooter, pwi);
        if (ammo.m_41619_() || !(ammo.m_41720_() instanceof ArrowItem arrowItem)) {
            return;
        }
        int piercing = enchLevel(net.minecraft.world.item.enchantment.Enchantments.f_44961_, crossbow);
        int multishot = enchLevel(net.minecraft.world.item.enchantment.Enchantments.f_44959_, crossbow);
        int shots = multishot > 0 ? 3 : 1;
        float[] angles = {0.0f, -10.0f, 10.0f};
        boolean infinity = enchLevel(net.minecraft.world.item.enchantment.Enchantments.f_44952_, crossbow) > 0;
        ...
        for (int i = 0; i < shots; i++) {
            AbstractArrow arrow = arrowItem.m_6394_(shooter.m_9236_(), ammo, shooter);
            arrow.m_36740_(SoundEvents.f_11840_);   // setSoundEvent(CROSSBOW_HIT)
            arrow.m_36793_(true);                   // setShotFromCrossbow
            if (piercing > 0) {
                arrow.m_36767_((byte) piercing);    // setPierceLevel（穿透）
            }
            ...
            arrow.m_20242_(true);
            arrow.m_6686_(rx, dy, rz, 2.4f, 1.0f);
    ...
    /** 读手持武器上的附魔等级（1.20.1：`EnchantmentHelper.getItemEnchantmentLevel`） */
    private static int enchLevel(net.minecraft.world.item.enchantment.Enchantment ench, ItemStack stack) {
        try {
            return net.minecraft.world.item.enchantment.EnchantmentHelper.m_44843_(ench, stack);
        } catch (Throwable ignored) {
            return 0;
        }
    }
     ...
     *
     * v1.2.0 实测四百九十二【箭矢不吃附魔】。反馈："看一下飞行时攻击能不能触发附魔效果。"
     *
     * 【根因·反编译实证】1.20.1 的 `ArrowItem.m_6394_(level, stack, shooter)` **只有三个参数、
     * 不收武器**（反编译：`new Arrow(level, shooter)` 之后只 `m_36878_(stack)` 设了拾取物），
     * 所以"附魔"这件事在原版里是由 **`BowItem.releaseUsing` 手工写上去的**——
     * 它造完箭后自己读武器附魔、逐条施加（见下）。我们旧版造完箭直接 `m_6686_` 射出去，
     * 这三条**从未被调用** → 飞行远战的箭矢在 1.20.1 上完全没有武器附魔。
     *
     * 【1.21.1 不存在这个问题】那边 `createArrow(level, stack, shooter, weapon)` 多一个武器参数、
     * 存进箭矢的 `firedFromWeapon`，命中时由 `EnchantmentHelper.modifyDamage` / `modifyKnockback`
     * 自行读取（无 `instanceof Player` 门控，实测确认）——本树只补 1.20.1。
     *
     * 【照抄原版 BowItem 的三条】逐条与原版同一算式，不做自创：
     * ```
     *   Power  (f_44988_) → m_36781_(m_36789_() + level * 0.5 + 0.5)   // 伤害
     *   Punch  (f_44989_) → m_36735_(level)                            // 击退
     *   Flame  (f_44990_) → m_20254_(100)                              // 点燃 5 秒
     * ```
     * 另按原版同款处理**无限附魔**（`f_44952_`）：有无限则不消耗背包里的箭
     * （原版是"创造或无限就不吃弹药"，我们照抄无限这一支）。
     */
    private static void shootArrow(EntityMaid shooter, LivingEntity target, ItemStack weapon,
                                   net.minecraft.world.item.ProjectileWeaponItem pwi) {
        ItemStack arrowStack = findArrow(shooter, pwi);
        if (arrowStack.m_41619_() || !(arrowStack.m_41720_() instanceof ArrowItem arrowItem)) {
            return;
        }
        AbstractArrow arrow = arrowItem.m_6394_(shooter.m_9236_(), arrowStack, shooter);
        double dx = target.m_20185_() - shooter.m_20185_();
        double dy = target.m_20188_() - shooter.m_20188_();
        double dz = target.m_20189_() - shooter.m_20189_();
        double dist = Math.sqrt(dx * dx + dz * dz);
        arrow.m_6686_(dx, dy + dist * 0.05, dz, 2.4f, 1.0f);
        arrow.m_20242_(true);

        // ↓↓↓ 原版 BowItem.releaseUsing 的附魔段（旧版整段缺失）
        try {
            int power = net.minecraft.world.item.enchantment.EnchantmentHelper.m_44843_(
                    net.minecraft.world.item.enchantment.Enchantments.f_44988_, weapon);
            if (power > 0) {
                arrow.m_36781_(arrow.m_36789_() + (double) power * 0.5 + 0.5);
            }
            int punch = net.minecraft.world.item.enchantment.EnchantmentHelper.m_44843_(
                    net.minecraft.world.item.enchantment.Enchantments.f_44989_, weapon);
            if (punch > 0) {
                arrow.m_36735_(punch);
            }
            if (net.minecraft.world.item.enchantment.EnchantmentHelper.m_44843_(
                    net.minecraft.world.item.enchantment.Enchantments.f_44990_, weapon) > 0) {
                arrow.m_20254_(100);
            }
        } catch (Throwable ignored) {
        }

        weapon.m_41622_(1, shooter, m -> m.m_21166_(EquipmentSlot.MAINHAND));
        // 无限附魔：有就不消耗箭（原版同款判据）
        boolean infinity = false;
        try {
            infinity = net.minecraft.world.item.enchantment.EnchantmentHelper.m_44843_(
                    net.minecraft.world.item.enchantment.Enchantments.f_44952_, weapon) > 0;
        } catch (Throwable ignored) {
        }
        if (!infinity) {
            arrowStack.m_41774_(1);
        }
        SoundEvent shoot = ForgeRegistries.SOUND_EVENTS.getValue(
                new ResourceLocation("minecraft", "entity.arrow.shoot"));
        if (shoot != null) {
    ...
    /**
     * 找箭：副手优先，其次按【武器自己的 supportedProjectiles 判据】在女仆背包找
     * （弓弩认箭、模组弹射武器认自己的弹药），最后退回背包里的任意箭。
     */
    private static ItemStack findArrow(EntityMaid maid, net.minecraft.world.item.ProjectileWeaponItem pwi) {
        try {
            ItemStack off = maid.m_21206_();
            if (!off.m_41619_() && off.m_41720_() instanceof ArrowItem) {
                return off;
            }
        } catch (Throwable ignored) {
        }
    ...
    /**
     * 弩 + 烟花火箭：照原版 `CrossbowItem.m_40889_(createProjectile)` 的烟花分支打出一枚真烟花。
     *
     ...
     * 于是直接落到 {@link #shootArrow}，而那里**只认 `ArrowItem`**：
     * 1.20.1 的 {@link #findArrow} 先看副手（烟花不是 ArrowItem，判不过），再去背包找箭；
     * 背包里没箭就返回空 → {@link #shootArrow} 直接 `return`，**一枪都放不出来**。
     * 也就是说"手上有弩有烟花，女仆却完全不开火"就是这么来的。
     *
     * 【烟花在本任务里的两种身份】烟花火箭在本任务里有两种身份：
     * ① **飞行燃料**——喂鞘翅推进用，发射路径自己造弹体
     *    （`MaidFlightCombatBehavior#launchFirework`），**不需要手持**；
     * ② **弩弹药**——原版口径（`EntityMaid.m_6298_` 副手是烟花就交出来当弹药）。
     ...
     *   与原版 `createProjectile` 的烟花分支逐字一致；末位 `true` = `shotAtAngle`，
     *   即**命中即爆、不给射手推力**（不是给滑翔加速的那种挂载烟花）；
     * - 瞄准照 TLM 自己的女仆弩口径（1.20.1 走原版 `m_4089_`/`shootProjectile`，
     *   1.21.1 走 `MixinCrossbowItem#shootCrossbowProjectile`）：方向取双方**眼高**之差，
     *   `setNoGravity(true)` 直线飞行，速度 = `clamp(距离/10, 1.6, 3.2)`、
     *   散布 = `1 - clamp(距离/100, 0, 0.9)`，音效 `CROSSBOW_SHOOT`（音高带 ±0.4 随机）；
     * - 耐久按原版烟花分支扣 **3**（射箭是 1）；
     ...
     * 会被 {@link FriendlyFireGuard}（`LivingAttackEvent` / `LivingHurtEvent`）拦下——
     * 女仆绝不会用烟花炸到主人。
     *
    ...
    private static boolean shootCrossbowFirework(EntityMaid shooter, LivingEntity target, ItemStack weapon) {
        try {
            ...
            double dz = target.m_20189_() - shooter.m_20189_();
            float dist = shooter.m_20270_(target);
            double velocity = net.minecraft.util.Mth.m_14036_(dist / 10.0f, 1.6f, 3.2f);
            double inaccuracy = 1.0f - net.minecraft.util.Mth.m_14036_(dist / 100.0f, 0.0f, 0.9f);
            ...
            net.minecraft.world.entity.projectile.FireworkRocketEntity rocket =
                    new net.minecraft.world.entity.projectile.FireworkRocketEntity(
                            shooter.m_9236_(), ammo, shooter,
                            shooter.m_20185_(), shooter.m_20188_() - 0.15, shooter.m_20189_(), true);
            rocket.m_20242_(true);
            // 注意：这里传的是 d/y/dz（眼高差）——与 TLM 女仆弩的口径一致，不是原版弓的抛物线抬升
