...
public final class MaidMealBridge {
    ...
    public static int eatByItemLogic(net.minecraft.world.entity.LivingEntity eater, ItemStack live) {
        if (eater == null || live == null || live.m_41619_()) {
            return 0;
        }
        try {
            int before = live.m_41613_();
            // m_5922_ = Item.finishUsingItem（ItemStack 的那个同名方法就是转发到它）
            live.m_41720_().m_5922_(live, eater.m_9236_(), eater);
            int consumed = Math.max(0, before - live.m_41613_());
            if (consumed == 0) {
    ...
    public static boolean onFeedCooldown(net.minecraft.world.entity.LivingEntity eater, ItemStack stack) {
        if (eater == null || stack == null || stack.m_41619_()) {
            return false;
        }
        try {
            ...
            // 玩家侧还有真正的物品冷却（m_36335_ = Player.getCooldowns、
            // m_41519_ = ItemCooldowns.isOnCooldown）；女仆侧靠上面的本地表
    ...
    private static String cdKey(net.minecraft.world.entity.LivingEntity eater, ItemStack stack) {
        ...
        try {
            ...
            id = rl == null ? String.valueOf(stack.m_41720_()) : rl.toString();
        } catch (Throwable ignored) {
            id = "?";
        }
        return eater.m_20148_() + "|" + id;
    }
