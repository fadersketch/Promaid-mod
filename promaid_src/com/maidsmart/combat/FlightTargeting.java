 ...
 *       AABB box = maid.searchDimension();          // ← 扫描盒由这里决定
 ...
 *       return brain.getMemory(NEAREST_VISIBLE_LIVING_ENTITIES)
 *                   .flatMap(m -&gt; m.findClosest(e -&gt; maid.canAttack(e)
 *                                                    &amp;&amp; maid.isWithinRestriction(e.blockPosition())));
 * </pre>
 * 而 {@code EntityMaid.searchDimension()} 是**按排班活动分流**的：
 * <pre>
 ...
 * </pre>
 * 也就是说我们两个空袭任务覆写的 50 格盒子**只在活动恰为 WORK 时参与**；其余时段走 idle
 * 任务的默认口径（{@code IMaidTask.searchDimension} 默认 = {@code inflate(searchRadius, 4.0,
 * searchRadius)}，**垂直硬编码 4 格**）。空袭是立体作战，可她一进非 WORK 活动段、扫描盒的
 * 垂直半径就掉回 4 格——她已经爬到敌人上方十几格开外，敌人**直接掉出盒子**。
 *
 * ── "飞太高 → 丢目标 → 往其他地方飞"是怎么连起来的 ──
 * 目标掉出可见列表后，{@code StartAttacking} 再也选不到它（它的目标来源就是那个列表）；
 * 而空袭行为自己的 canStillUse 又要求"当前有目标"——目标一丢，行为立刻停止、状态被
 * {@code forget} 整清，她在高空保持滑翔却**没有任何朝向指令**，于是顺着惯性飘走。这就是
 * 用户看到的"直接脱离了战场"。
 *
 * ── 现在的口径（本类）──
 * 把索敌从 TLM 那条链路上**完全摘出来**，每 tick 自己维持，三件事：
 * <ol>
 *   <li>**自己扫**以自身碰撞箱为中心、半径 {@link #RANGE} 格的<b>球</b>（AABB 膨胀做粗筛、
 *       距离平方做精筛），**不看排班活动**——所以她爬多高都算；</li>
 *   <li>目标选定后**每 tick 回写** {@code ATTACK_TARGET} / {@code LOOK_TARGET}——TLM 那边
 *       什么时候擦、按什么判据擦都不影响我们，下一 tick 就补回来；</li>
 *   <li>已经锁定的目标只要"还活着 + 还能攻击 + 还在活动范围内 + 仍在 {@link #HOLD_RANGE}
 *       内"就**继续用它、不再要求视线**。这一条正是"持续瞄准"的实质：她飞高时与地面敌人
 *       之间的视线经常被地形/树叶/怪物身体挡住，旧口径下这会让她把目标判成"不可见"而放弃；
 *       现在视线只作为**新锁定**的门槛，不影响**已锁定目标**的维持——所以她会一直朝着那个
 *       方向飞回去，而不是原地飘走。</li>
 * </ol>
 *
 * 【与 TLM "隔墙不出手"没有任何冲突】新锁定仍然要过一道视线 raycast
 * （{@code SelfPreservationBehavior.hasSight}，与实测五百零五给自保近战补的是同一个方法），
 * 所以墙后的怪不会被**新**锁定；改变的只是"已经打起来的目标不会因为她飞高、视线被挡一下就
 * 被丢掉"。
 *
 * ── v1.2.0 实测五百二十四（本类的第二处修正，来自实测日志）──
 * 上一版（五百二十一）留下两个会把"持续瞄准"重新打断的洞，都在本轮补齐：
 * <ol>
 *   <li>{@link #scan} 的视线门当年误用了 {@code Sensor.isEntityAttackable}——它**内含
 *       `range(16.0)`**，于是"50 格索敌"实际只能在 16 格内生效。她一旦被起飞弧线推到 16 格外
 *       就再也拿不回目标（日志里大片 `候选=3 → 可攻击=1 → 有视线=0 → 选中=-`）；</li>
 *   <li>维持锁定用的距离上限与新锁定共用 50 格——**起飞那口气就把她推出 50 格**
 *       （实测：锁定时 7.1 格，放烟花起飞 2 秒后目标就没了）。现拆成
...
public final class FlightTargeting {
     ...
     * 状态被 forget 清空，她在高空保持滑翔却**没有任何朝向指令** → 顺着惯性飘出战场。
     * 这正是"飞得特别高就失去方向"的来源。
     *
     * 所以：**50 只管发现，打起来之后由 128 兜底**（起飞弧线最坏约 60 格，
     * 见 1.20.1/1.21.1 各一份实测日志；128 有约两倍余量，同时仍然能防住
     * "追着跑出战场"）。真正该收尾的情形（目标死了 / 打不着 / 越出活动范围）
     * 由 {@link #keepable} 里其余几条负责，与新锁定同一口径。
     */
     ...
     * {@code checkExtraStartConditions}（就是本类的 ③ 那一步）。若不限频，"没有敌人"的空袭
     * 女仆会每 tick 扫一次 50 格 AABB 盒——白烧性能。有了它，没锁定时最多每 0.5 秒扫一次。
    ...
    public static LivingEntity resolve(EntityMaid maid) {
        ...
        try {
            ...
            //    重新索敌不必再等最多 0.5 秒的限频（限频只为"没敌人时别空扫"，不该在
            //    交战中拖延重新锁定）。
    ...
    private static boolean keepable(EntityMaid maid, LivingEntity e) {
        try {
            if (e == null || !e.m_6084_() || e.m_9236_() != maid.m_9236_()) {
                return false;
            }
            ...
            if (!maid.m_21444_(e.m_20183_())) {
                return false;
            }
            return maid.m_20280_(e) <= HOLD_RANGE_SQR;
        } catch (Throwable ignored) {
            return false;
        }
    }
    ...
    private static LivingEntity brainTarget(EntityMaid maid) {
        try {
            java.util.Optional<LivingEntity> mem = maid.m_6274_().m_21952_(MemoryModuleType.f_26372_);
            if (mem.isPresent() && mem.get().m_6084_()) {
                return mem.get();
            }
        } catch (Throwable ignored) {
        }
        LivingEntity t = maid.m_5448_();
        return t != null && t.m_6084_() ? t : null;
    }
    ...
    private static LivingEntity scan(EntityMaid maid) {
        try {
            net.minecraft.world.phys.AABB box = maid.m_20191_().m_82377_(RANGE, RANGE, RANGE);
            LivingEntity best = null;
            double bestSqr = Double.MAX_VALUE;
            for (LivingEntity e : maid.m_9236_().m_6443_(LivingEntity.class, box, x -> true)) {
                if (e == maid || !e.m_6084_()) {
                    continue;
                }
                if (FriendlyFireGuard.isFriendly(maid, e)) {
                ...
                // 想当然地把它当成"视线判定"。反编译实证（`Sensor.m_26803_` + `TargetingConditions`）：
                //
                //     isEntityAttackable(a, b) {
                //         if (brain.hasMemoryValue(ATTACK_TARGET, b)) return C_IGNORE_SIGHT.test(a, b);
                //         return C.test(a, b);          // C = forNonCombat().range(16.0)
                //     }
                //
                // **`C` 带着 `range(16.0)`**——`TargetingConditions.m_26885_` 里那句
                // `d = a.distanceToSqr(b); if (d > (16 * visibility)²) return false;`。
                // 也就是说这一环真正卡的是"**16 格以内**"，视线只是顺带。于是：
                // 我们扫的是 50 格球、候选也都捞进来了（日志 `候选=3 存活=2 可攻击=1`），
    ...
    private static void writeBrain(EntityMaid maid, LivingEntity target) {
        try {
            maid.m_6274_().m_21886_(MemoryModuleType.f_26372_, java.util.Optional.of(target));
