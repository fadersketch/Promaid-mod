 ...
 * （风免刻意不管"她推自己"，见 {@link FriendlyWindGuard}），实测就是"轻松飞出 100 格"。
 *
 * ── 现在的口径（一句话）──
 * <pre>
 ...
 * 原版挂载烟花对骑手的推力**不是一次性的**（1.20.1 `FireworkRocketEntity.m_8119_` 字节码实证）：
 * <pre>
...
public final class MaidRiptideBoost {
        ...
        try {
            ...
            try {
                net.minecraft.sounds.SoundEvent snd = lvl >= 3
                        ...
                        : (lvl == 2 ? net.minecraft.sounds.SoundEvents.f_12518_
            ...
            com.maidsmart.tool.PromaidLog.log(channel, com.maidsmart.tool.PromaidLog.nameOf(maid)
                    ...
                    + (target == null ? "" : "，距敌 " + fmt(maid.m_20270_(target)) + " 格") + "）");
            return true;
        } catch (Throwable ignored) {
            return false;
        }
    ...
    public static boolean clear(EntityMaid maid) {
        try {
            return maid != null && THRUST.remove(maid.m_20148_()) != null;
        } catch (Throwable ignored) {
            return false;
        }
    }
    ...
    public static boolean isThrusting(EntityMaid maid) {
        try {
            return maid != null && THRUST.containsKey(maid.m_20148_());
        } catch (Throwable ignored) {
            return false;
        }
    }
    ...
    private static void pushOnce(EntityMaid maid, Thrust t) {
        ...
        maid.m_20256_(new Vec3(v.f_82479_ * GAIN + look.f_82479_ * add,
