...
public final class NeutralThreatDriver {
    ...
    private static void drive(ServerLevel level, EntityMaid maid) {
        ...
        // 玩家的 `Abilities.f_35934_`（invulnerable）为 true → 该判定 false → **怪物根本
        // 不会把创造模式的玩家当成敌人**，getTarget 恒为 null → 本驱动静默。切回生存
        ...
        if (!com.maidsmart.config.MaidSmartConfig.COMBAT_AUTO_SWITCH.get()) {
            boolean mine = ASSIGNED_TARGETS.remove(maid.m_20148_()) != null;
            ATTACK_CDS.remove(maid.m_20148_());
            if (mine) {
                try {
        ...
        if (dist <= STRIKE_DIST) {
            ...
            // v1.2.0 实测五百零五【隔墙挥空根因】：本兜底直调 m_7327_，绕过了 TLM 原生
            // 近战那道视线门。TLM `MaidMeleeAttack`（两树反编译实证）的出手条件是
            ...
            // （反馈："墙壁后有怪物酒狐就会疯狂地攻击墙壁"）。
            // 修法：出手前补一次视线判定（复用项目现有的那条 raycast 工具，不新造轮子）。
            // 【隔墙不罚站】注意这里只在"挥砍"这一步拦——下面的导航分支照常执行，
            // 她仍会朝怪走过去、绕到能看见的位置再打，追击欲望不受影响。
