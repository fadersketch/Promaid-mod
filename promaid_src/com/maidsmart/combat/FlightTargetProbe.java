 ...
 * （夜间 REST/IDLE 等）扫描盒子来自**空闲任务**的默认口径（水平 = 限制半径 m_21535_，
 * 垂直硬编码 4.0），我们那个 50 根本不参与。这足以解释"范围仍然很奇怪"。
 *
 ...
 *
 * 另外单独输出 `原版可判` = `Sensor.m_26803_` 的通过数，但**它含 `range(16.0)`**
...
public final class FlightTargetProbe {
    ...
    /**
     * 每 tick 由空袭行为调用（仅空袭任务、且无目标时才真正扫描）。
     *
     * @param maid 空袭女仆
     */
    public static void tick(EntityMaid maid) {
        ...
        try {
            ...
            Long last = LAST.get(maid.m_20148_());
            if (last != null && now - last < LOG_INTERVAL) {
                return;
            }
            // 有目标就说明锁上了，不用记（这条日志专治"锁不到"）
            java.util.Optional<net.minecraft.world.entity.LivingEntity> cur =
                    maid.m_6274_().m_21952_(net.minecraft.world.entity.ai.memory.MemoryModuleType.f_26372_);
            if (cur.isPresent() && cur.get().m_6084_()) {
                return;
            }
            LAST.put(maid.m_20148_(), now);
            probe(maid);
    ...
    private static void probe(EntityMaid maid) {
        ...
        java.util.List<net.minecraft.world.entity.LivingEntity> cands = maid.m_9236_().m_6443_(
        ...
        for (net.minecraft.world.entity.LivingEntity e : cands) {
            ...
            inSphere++;
            if (!net.minecraft.world.entity.ai.sensing.Sensor.m_26803_(maid, e)) {
        ...
        com.maidsmart.tool.PromaidLog.log("空袭索敌", String.format(
                ...
                taskUid(maid),
                activityName(maid),
                box.m_82362_(), box.m_82376_(), box.m_82385_(),
    ...
    /** 实体描述：类型 + 距离用的可读名（诊断用，只读 getType） */
    private static String desc(net.minecraft.world.entity.LivingEntity e) {
        try {
            ...
            return key == null ? e.m_6095_().toString() : key.toString();
        } catch (Throwable t) {
            return "?";
        }
    }
