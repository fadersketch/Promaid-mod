...
public class SelfPreservationBehavior extends Behavior<EntityMaid> {
    ...
    private static void logBuildRefused(EntityMaid maid, String reason) {
        try {
            ...
            Long last = BUILD_REFUSE_SINCE.get(maid.m_20148_());
            if (last != null && now - last < 100L) { // 5 秒
                return;
            }
            BUILD_REFUSE_SINCE.put(maid.m_20148_(), now);
     ...
     * 可窒息方块里吃 1 点"（{@code LivingEntity.m_6075_} 实证），所以真被闷住时
    ...
    private static float recentSuffocateDamage(EntityMaid maid) {
        SuffocateRecord rec = SUFFOCATE_DAMAGE.get(maid.m_20148_());
        if (rec == null) {
            return 0.0f;
        }
        if (maid.m_9236_().m_46467_() - rec.tick() > SUFFOCATE_WINDOW_TICKS) {
            SUFFOCATE_DAMAGE.remove(maid.m_20148_());
            return 0.0f;
        }
        return rec.amount();
    }
    ...
    private static boolean suffocateDiagDue(EntityMaid maid, long now) {
        Long last = SUFFOCATE_DIAG_SINCE.get(maid.m_20148_());
        if (last != null && now - last < SUFFOCATE_DIAG_INTERVAL) {
            return false;
        }
        SUFFOCATE_DIAG_SINCE.put(maid.m_20148_(), now);
        return true;
    }
    ...
    public static void onMaidHurt(net.minecraftforge.event.entity.living.LivingHurtEvent event) {
        ...
        try {
            ...
            if (srcS != null && isSuffocateMsgId(srcS.m_19385_())) {
                long nowS = maid.m_9236_().m_46467_();
                java.util.UUID idS = maid.m_20148_();
    ...
    protected boolean m_6114_(ServerLevel level, EntityMaid maid) {
        ...
        // 启动（setTask→refreshBrain 后新实例永不 tick），那样 m_6725_ 里的
        // PRESERVE_TAG 残留清理就永远跑不到。豁免统一放 m_6725_（行为常驻、
        // 每 tick 都能清理）。
    ...
    protected void m_6725_(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        // 打断烟花推进与盘旋（与 AutoCombatSwitch 里"飞行任务自主战斗不介入"
        // 同一口径）。这里做成【硬闸】：不但不进会话，已经在会话里的（切飞行任务
        // 前触发的）也立刻安静退出——否则 PRESERVE_TAG 残留会让排班/参战/搭方块
        ...
        if (airborne) {
            ...
            boolean hadTag = maid.getPersistentData().m_128471_(PRESERVE_TAG);
    ...
    private boolean hasLocalEscape(EntityMaid maid) {
        try {
            ...
            if (pearl != null && com.maidsmart.tool.MaidExtraContainer.contains(maid, s -> s.m_41720_() == pearl)) {
                return true;
            }
        } catch (Exception ignored) {
        }
    ...
    private static boolean isGoldenAppleStack(ItemStack s) {
        if (s == null || s.m_41619_() || s.m_41720_() == null) {
            return false;
        }
        return s.m_41720_() == net.minecraft.world.item.Items.f_42436_    // golden_apple
                || s.m_41720_() == net.minecraft.world.item.Items.f_42437_; // enchanted_golden_apple
    }
    ...
    private boolean useGoldenApple(EntityMaid maid) {
        try {
            ...
            // v1.2.4 实测六百三十九【先判背包，再判精妙背包】：同上（着火吃那条路）
            com.maidsmart.tool.MaidExtraContainer.pull(maid, SelfPreservationBehavior::isGoldenAppleStack, 1);
            IItemHandler inv = maid.getAvailableBackpackInv();
            int bestSlot = -1;
            boolean enchanted = false;
