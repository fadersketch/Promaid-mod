...
/**
 * v1.2.0（1.20.1）实测四百六十九：飞行作战——免疫"鞘翅撞击伤害"（用户要求的后门）。
 *
 ...
 * 比绑定混淆后的 SRG 常量名更耐版本变化。
 *
 * 事件用 `LivingAttackEvent`（Forge 1.20.1 没有 NeoForge 的 LivingIncomingDamageEvent）：
 * 它在伤害源头、可取消，最贴合"免疫"语义（与 FriendlyFireGuard 同一套路）。
 *
