...
public final class MaidBroomKit {
    ...
    public static boolean isBroomItem(ItemStack stack) {
        ...
        try {
            return stack.m_41720_() instanceof com.github.tartaricacid.touhoulittlemaid.item.ItemBroom;
        } catch (Throwable ignored) {
            return false;
        }
    }
    ...
    public static boolean hasBroomItem(EntityMaid maid) {
        ...
        try {
            if (isBroomItem(maid.m_21205_()) || isBroomItem(maid.m_21206_())) {
                return true;
            }
        } catch (Throwable ignored) {
        }
    ...
    public static EntityBroom ridingBroom(EntityMaid maid) {
        try {
            ...
            return maid.m_20202_() instanceof EntityBroom b ? b : null;
        } catch (Throwable ignored) {
            return null;
        }
    }
     ...
     * {@link CombatWorkRange}）——我们**直接读 {@code maid.m_21535_()}**，所以吃得到
    ...
    public static Vec3 clampToHome(EntityMaid maid, Vec3 desired) {
        ...
        try {
            ...
            double cx = c.m_123341_() + 0.5;
            double cz = c.m_123343_() + 0.5;
            double dx = desired.f_82479_ - cx;
            double dz = desired.f_82481_ - cz;
            double h = Math.sqrt(dx * dx + dz * dz);
            if (h <= r || h <= 1.0E-4) {
                return desired;
            }
            double k = r / h;
            return new Vec3(cx + dx * k, desired.f_82480_, cz + dz * k);
        } catch (Throwable ignored) {
            return desired;
        }
    }
    ...
    public static boolean isBroomAirborne(EntityMaid maid) {
        try {
            return isBroomTask(maid) && isRidingBroom(maid) && !maid.m_9236_().m_5776_();
        } catch (Throwable ignored) {
            return false;
        }
    }
    ...
    public static boolean homeRestricted(EntityMaid maid) {
        try {
            ...
            return maid != null && maid.m_21536_()
