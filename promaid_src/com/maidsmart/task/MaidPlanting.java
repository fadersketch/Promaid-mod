...
public final class MaidPlanting {
            ...
            for (net.minecraft.server.level.ServerLevel lvl : server.m_129785_()) {
                ...
                for (net.minecraft.world.entity.Entity e : lvl.m_8583_()) {
                    ...
                    try {
                        ...
                        tryRestoreHand(maid, maid.m_19879_(), lvl.m_46467_());
    ...
    private static boolean areaClearOfCollision(ServerLevel level, net.minecraft.core.BlockPos center) {
        for (int ddx = -1; ddx <= 1; ddx++) {
            for (int ddz = -1; ddz <= 1; ddz++) {
                net.minecraft.core.BlockPos q = center.m_7918_(ddx, 0, ddz);
                if (!level.m_8055_(q).m_60742_(level, q,
    ...
    private static boolean hasGrowthSpace(ServerLevel level, net.minecraft.core.BlockPos spot) {
        for (int i = 1; i <= TRUNK_CLEARANCE; i++) {
            net.minecraft.core.BlockPos q = spot.m_7918_(0, i, 0);
            net.minecraft.world.level.block.state.BlockState st = level.m_8055_(q);
            if (st.m_60795_()) {
                continue; // isAir
            }
            if (st.m_204336_(LEAVES_TAG)) {
                continue; // 树叶可被树干穿过（原版 isFree 口径）
            }
            if (st.m_60742_(level, q,
                    net.minecraft.world.phys.shapes.CollisionContext.m_82749_()).m_83281_()) {
    ...
    private static void tryBonemealSapling(ServerLevel level, EntityMaid maid) {
        ...
        for (int dx = -RADIUS; dx <= RADIUS; dx++) {
            for (int dz = -RADIUS; dz <= RADIUS; dz++) {
                for (int dy = -2; dy <= 2; dy++) {
                    ...
                    if (!(st.m_60734_() instanceof net.minecraft.world.level.block.SaplingBlock)) {
                        continue;
                    }
                    net.minecraft.resources.ResourceLocation key =
                            ForgeRegistries.BLOCKS.getKey(st.m_60734_());
                    if (key != null && "dark_oak_sapling".equals(key.m_135815_())) {
                        continue; // 深色橡树单株永不生长，不浪费骨粉
                    }
                    if (!hasGrowthSpace(level, p)) {
                        continue; // 树干上方被实心方块挡死——骨粉也长不成
                    }
                    double d = maid.m_20275_(p.m_123341_() + 0.5, p.m_123342_() + 0.5,
                            p.m_123343_() + 0.5);
                    if (d < bestD) {
                        bestD = d;
                        best = p;
        ...
        ItemStack boneStack = maid.m_21205_();
        boolean ok = net.minecraft.world.item.BoneMealItem.m_40627_(boneStack, level, best);
        if (ok) {
            // growCrop 不放 1505（原版粒子在 useOn 路径）——每次施肥都给粒子反馈
            level.m_46796_(1505, best, 15);
            maid.m_6674_(net.minecraft.world.InteractionHand.MAIN_HAND);
            LOGGER.info("bone meal sapling: maid={} pos={}", maid.m_20148_(), best);
        }
    }
    ...
    public static net.minecraft.world.item.Item boneMealItem() {
        if (!BONE_MEAL_RESOLVED) {
            try {
                BONE_MEAL_CACHE = ForgeRegistries.ITEMS.getValue(
    ...
    public static boolean equipBoneMeal(EntityMaid maid, int id, long now) {
        ...
        try {
            net.minecraftforge.items.IItemHandlerModifiable hands =
                    (net.minecraftforge.items.IItemHandlerModifiable) maid.getHandsInvWrapper();
            ItemStack main = maid.m_21205_();
            if (!main.m_41619_() && main.m_41720_() == boneMeal) {
    ...
    private static void restoreNow(EntityMaid maid) {
        try {
            ...
            if (!cur.m_41619_() && cur.m_41720_() == bone) {
            ...
            if (!held.m_41619_()) {
                hands.setStackInSlot(1, ItemStack.f_41583_);
                if (hands.getStackInSlot(0).m_41619_()) {
    ...
    public static boolean isSaplingItem(ItemStack stack) {
        try {
            ...
            if (!(stack.m_41720_() instanceof net.minecraft.world.item.BlockItem)) {
                return false;
            }
            Block b = ((net.minecraft.world.item.BlockItem) stack.m_41720_()).m_40614_();
            if (b == null) {
                return false;
            }
            ...
            return stack.m_41720_() instanceof net.minecraft.world.item.ItemNameBlockItem
                    && b.m_49966_().m_204336_(SAPLINGS_TAG);
        } catch (Exception ignored) {
            return false;
        }
    }
