...
public class MaidStrollBehavior extends Behavior<EntityMaid> {
     ...
     * 另一半是「她到底有没有在散步」得能一眼看出来——所以把门禁从 {@link #m_6114_}
     * 里抽出来，{@code /maid_smart stroll check} 念的就是这几条。
     */
    ...
    /**
     * 逐条算一遍门禁（true = 这一条把她挡住了）。{@link #m_6114_} 与自检**共用**这一份，
     * 免得「自检说会散步、实际不散步」这种口径漂移。
     *
    ...
    public static boolean[] gates(ServerLevel level, EntityMaid maid) {
        ...
        try {
            ...
            // 自保中不散步
            g[1] = maid.getPersistentData().m_128471_(
            ...
            try {
                g[4] = maid.m_6274_().m_21952_(MemoryModuleType.f_26370_).isPresent();
            } catch (Throwable t) {
                g[4] = true;
            }
            // 正在接战不打扰
            try {
                ...
                g[5] = atk.isPresent() && atk.get().m_6084_();
            } catch (Throwable ignored) {
            }
            // 坐姿/乘骑不散步（getVehicle 实测为 m_20202_——MaidRunOne 字节码实证）
            g[6] = maid.isMaidInSittingPose() || maid.m_20202_() != null;
    ...
    public static long nextStrollIn(ServerLevel level, EntityMaid maid) {
        ...
        return Math.max(0L, next - level.m_46467_());
    }
    ...
    protected void m_6735_(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        float restrictR = maid.m_21535_();     // getRestrictRadius
        ...
        boolean restricted = maid.m_21536_();
        if (restricted) {
    ...
    public static BlockPos pickLongRun(ServerLevel level, EntityMaid maid, int max, int minRun) {
        ...
        for (int[] d : dirs) {
            ...
            BlockPos last = null;
            for (int k = 1; k <= max; k++) {
                BlockPos p = base.m_7918_(d[0] * k, 0, d[1] * k);
                BlockPos stand = null;
                // 台阶上下各让一格：上坡/下坡也算走得过去
                for (int dy = 1; dy >= -1; dy--) {
                    BlockPos c = p.m_7918_(0, dy, 0);
