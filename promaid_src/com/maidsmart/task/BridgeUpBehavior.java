...
public class BridgeUpBehavior extends Behavior<EntityMaid> {
    ...
    protected boolean m_6114_(ServerLevel level, EntityMaid maid) {
        ...
        // 主人在上方走垂直/斜上搭高，主人在下方走下行台阶（逐级降低高度），
        // 平路/缺口照旧。
    ...
    protected void m_6725_(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        if (this.walkOnTicks > 0) {
            ...
            boolean arrived = Math.floor(maid.m_20185_()) == Math.floor(this.walkOnX)
                    ...
                    && (this.walkOnY < maid.m_20186_() - 0.01
     ...
     * 逐级降低高度接近主人（与挖矿/伐木 descendStep 同思路，但方向朝主人而非目标方块）。
    ...
    private boolean tryDescendStep(ServerLevel level, EntityMaid maid, double hx, double hz, double hDist) {
        ...
        for (double[] d : dirs) {
            ...
            net.minecraft.world.level.block.state.BlockState supportState = level.m_8055_(support);
