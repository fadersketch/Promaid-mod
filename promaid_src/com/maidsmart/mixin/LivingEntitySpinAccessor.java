 ...
 * 反编译实证：1.20.1 的 `Entity.m_20115_(setSharedFlag)` 写的是 **`Entity.f_19805_`**
 * （DATA_SHARED_FLAGS_ID，滑翔位就走它）；而 `isAutoSpinAttack`（`m_21209_`）读的是
 * **`LivingEntity.f_20909_`**（DATA_LIVING_ENTITY_FLAGS）——**两个不同的 SynchedEntityData**。
 * 所以滑翔那套 invoker 够不到旋转突进，必须按 LivingEntity 单独暴露。
 *
 * 【tick 数同源】`f_20938_`（1.21.1 叫 `autoSpinAttackTicks`）也在 LivingEntity 上，
 * 由 `LivingEntity.tick` 每 tick 递减并调 `m_21071_`/`checkAutoSpinAttack` 做扫掠——
 * 女仆置位后即自动获得原版的整套突进行为。
 */
@Mixin(LivingEntity.class)
public interface LivingEntitySpinAccessor {

    /** 读旋转突进剩余 tick（LivingEntity.f_20938_） */
    @Accessor("f_20938_")
