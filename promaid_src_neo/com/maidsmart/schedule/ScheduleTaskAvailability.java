...
public final class ScheduleTaskAvailability {
    ...
    private static final TagKey<Block> LOGS_TAG = net.minecraft.tags.TagKey.create(net.minecraft.core.registries.Registries.BLOCK, ResourceLocation.parse("minecraft:logs"));
    private static final TagKey<Block> BAMBOO_TAG = net.minecraft.tags.TagKey.create(net.minecraft.core.registries.Registries.BLOCK, ResourceLocation.parse("minecraft:bamboo_blocks"));
            ...
            for (int dz = -r; dz <= r; dz++) {
                for (int dy = -down; dy <= up; dy++) {
                    ...
                    if (woods.contains(keyOf(b))
                            || (tagAuto && (st.is(LOGS_TAG) || st.is(BAMBOO_TAG)))) {
                        return true;
                    }
                }
            }
        ...
        for (int dx = -FARM_RADIUS; dx <= FARM_RADIUS; dx++) {
            for (int dz = -FARM_RADIUS; dz <= FARM_RADIUS; dz++) {
                for (int dy = -1; dy <= 1; dy++) {
                    BlockPos base = c.offset(dx, dy, dz);
                    if (!level.isLoaded(base)) {
                        continue;
                    }
                    BlockPos crop = c.offset(dx, dy + 1, dz);
                    try {
                        if (farm.canHarvest(maid, crop, level.getBlockState(crop))) {
                            return true;
                        }
    ...
    private static java.util.List<ItemStack> collectSeeds(EntityMaid maid, IFarmTask farm) {
        ...
        try {
            ...
            for (int slot = 0; slot < inv.getSlots(); slot++) {
                ...
                if (!stack.isEmpty() && farm.isSeed(stack)) {
                    seeds.add(stack.copy()); // 副本，防后续同刻被清
                }
            }
        } catch (Throwable ignored) {
        }
