 ...
 * （本树就是 {@code stillValid} 这个名字），其中一条是「玩家到她 ≤ 4 格」。
 ...
 * 边界取的是**原版自己的实体同步距离**（{@code EntityType.clientTrackingRange()}，
...
public final class RemoteMaidGui {
     ...
     * 远程开界面的距离上限（格）。
     * <p>
     * 【为什么封顶 48】原版实体同步距离是按**区块**算的（{@code clientTrackingRange()} × 16），
    ...
    public static double remoteRange(EntityMaid maid) {
        try {
            ...
            int chunks = maid.getType().clientTrackingRange();
