...
public final class BeyondBindingStore extends SavedData {
    ...
    private BeyondBindingStore(CompoundTag tag) {
        net.minecraft.nbt.ListTag maids = tag.contains(KEY_MAIDS, 9)
                ? tag.getList(KEY_MAIDS, 9) : new net.minecraft.nbt.ListTag();
        for (int i = 0; i < maids.size(); i++) {
            net.minecraft.nbt.CompoundTag e = maids.getCompound(i);
            String id = e.getString(KEY_MAID_ID);
            net.minecraft.nbt.ListTag terms = e.contains(KEY_INTERFACES, 9)
                    ? e.getList(KEY_INTERFACES, 9) : new net.minecraft.nbt.ListTag();
            List<long[]> list = new ArrayList<>();
            for (int j = 0; j < terms.size(); j++) {
                String s = terms.getString(j);
    ...
    public static BeyondBindingStore get(ServerLevel level) {
        return level.getDataStorage().computeIfAbsent(new net.minecraft.world.level.saveddata.SavedData.Factory<BeyondBindingStore>(BeyondBindingStore::new,
                        (t, registries) -> new BeyondBindingStore(t),
                        net.minecraft.util.datafix.DataFixTypes.SAVED_DATA_RANDOM_SEQUENCES), DATA_NAME);
    }
    ...
    public CompoundTag save(CompoundTag tag, net.minecraft.core.HolderLookup.Provider registries) {
        ...
        for (java.util.Map.Entry<String, List<long[]>> e : this.bindings.entrySet()) {
            ...
            entry.putString(KEY_MAID_ID, e.getKey());
            net.minecraft.nbt.ListTag terms = new net.minecraft.nbt.ListTag();
            for (long[] arr : e.getValue()) {
                terms.add(net.minecraft.nbt.StringTag.valueOf(
                        arr[0] + "," + arr[1] + "," + arr[2]));
            }
            entry.put(KEY_INTERFACES, terms);
            maids.add(entry);
        }
        tag.put(KEY_MAIDS, maids);
        return tag;
    }
}
