...
public class SmartPlaceTool implements ITool<SmartPlaceTool.Result> {
    ...
    public LLMCallback onCall(String toolId, Result result, LLMCallback callback) {
        ...
        try {
            blockId = ResourceLocation.tryParse(blockStr); // parse（非法 id 抛异常→兜底）
        } catch (Exception ex) {
        ...
        Item blockItem = net.minecraft.core.registries.BuiltInRegistries.ITEM.get(blockId);
        ...
        for (int i = 0; i < inv.getSlots(); i++) {
            ...
            if (!s.isEmpty() && s.getItem() == blockItem) {
                slot = i;
                break;
            }
        }
        ...
        if (result.x() != Integer.MIN_VALUE && result.y() != Integer.MIN_VALUE && result.z() != Integer.MIN_VALUE) {
            ...
            target = foot.offset(0, 0, 0);
            // 视线方向（水平）：女仆 look 向量 → 前方块
            double yaw = Math.toRadians(maid.getYRot()); // getYRot
            int dx = (int) Math.round(-Math.sin(yaw));
            int dz = (int) Math.round(Math.cos(yaw));
            if (dx == 0 && dz == 0) {
                dz = 1;
            }
            target = foot.offset(dx, 0, dz);
        }
        // 距离检查（别隔老远隔山打牛）——distSqr = distSqr（平方距离 > 144 = 12 格）
        if (maid.blockPosition().distSqr(target) > 144) {
        ...
        for (int i = 0; i < 8 && count > 0; i++) {
            BlockPos p = target.offset(0, i / 4 == 0 ? 0 : 1, 0); // 前 4 个放目标行，后 4 个上一行
            BlockState cur = level.getBlockState(p);
            // 可放条件：空气/水（流体空=空气或水？用 isEmpty）或 replace=true 且方块本身可替换
            // （canBeReplaced = canBeReplaced：水/草丛/雪/藤蔓等，replace=true 时覆盖它们）
            boolean ok = cur.isAir()
                    || (!cur.getFluidState().isEmpty()
                        && cur.getFluidState().is(net.minecraft.tags.FluidTags.WATER))
                    || (result.replace() && cur.canBeReplaced());
            if (!ok) {
                fail++;
                continue;
            }
            ...
            level.setBlock(p, ((BlockItem) blockItem).getBlock().defaultBlockState(), 3); // getBlock().defaultBlockState()
            placed++;
            count--;
        }
