 ...
 * （{@code CapabilityBackpackWrapper}），**不是** Forge / NeoForge 的 {@code ITEM_HANDLER}
 * ——自己找就得硬编码它的类名、版本与内部方法名。TLM 已经把这层做完了，而且顺带覆盖了旅行者
 * 背包、以及将来任何注册进它那套体系的容器；我们只调它的公开 API，越少自己摸越不容易碎。
 *
...
public final class MaidExtraContainer {
     ...
     */
    private static int firstMatch(net.neoforged.neoforge.items.IItemHandler inv, Predicate<ItemStack> filter) {
    ...
    private static void logPull(EntityMaid maid, ItemStack got, int maxCount) {
        try {
            ...
            String id;
            try {
                id = String.valueOf(net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(got.getItem()));
            } catch (Throwable ignored) {
                id = got.getItem().toString();
            }
            PromaidLog.log("额外容器", nameOf(maid) + " 她身上没有 → 从精妙背包/旅行者背包取来 "
                    + got.getCount() + "x" + id + (maxCount == -1 ? "（整组）" : ""));
        } catch (Throwable ignored) {
        }
    }
    ...
    public static ItemStack overflow(EntityMaid maid, ItemStack rest) {
        if (maid == null || rest == null || rest.isEmpty()) {
            return rest;
        }
        if (!enabled()) {
            return rest;
        ...
        for (int i = 0; i < refs.size() && !rest.isEmpty(); i++) {
        ...
        if (moved > 0) {
            PromaidLog.log("额外容器", nameOf(maid) + " 自己背包塞不下 → 精妙背包/额外容器收下 "
                    + moved + " 件" + (rest.isEmpty() ? "（全部收下）" : "（还剩 " + rest.getCount() + " 件）"));
        }
        return rest;
    }
