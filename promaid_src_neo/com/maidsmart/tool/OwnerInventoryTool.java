...
public class OwnerInventoryTool implements ITool<OwnerInventoryTool.Result> {
    ...
    public LLMCallback onCall(String toolId, Result result, LLMCallback callback) {
        ...
        // 主人判空（未认领/离线时 getOwner 可能为 null 或非玩家实体）
        ...
        int listed = 0;
        for (int i = 0; i < inv.getContainerSize() && i <= 40; i++) {
            ItemStack stack = inv.getItem(i);
            if (stack.isEmpty()) {
                continue;
            }
            net.minecraft.resources.ResourceLocation rl = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(stack.getItem());
