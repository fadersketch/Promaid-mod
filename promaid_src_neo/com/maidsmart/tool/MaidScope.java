 ...
 * </ul>
 * <b>故意不用 {@code getOwner()}</b>（= TLM 那个只返回"在线的那个玩家实体"的方法）：
...
public final class MaidScope {
     ...
     * 需要"活着"的调用点自己再判 {@code isAlive()}。
     */
    public static boolean owned(EntityMaid maid) {
        try {
            return maid != null && maid.getOwnerUUID() != null;
    ...
    public static void noteDowngrade(EntityMaid maid) {
        try {
            ...
            PromaidLog.log("降级", PromaidLog.nameOf(maid)
                    + " 是无主女仆（没有主人）→ promaid 的全部改动对她不生效，保持 TLM 原版行为");
        } catch (Throwable ignored) {
        }
    }
