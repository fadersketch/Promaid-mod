...
public final class MaidFreeFlightGotoCommand {
    ...
    public static void register(com.mojang.brigadier.CommandDispatcher<CommandSourceStack> dispatcher) {
        // 【实测六百八十七】补权限门：另两条命令（MaidArmyCommand / MaidResyncCommand）都是
        // `.requires(src -> src.hasPermission(2))`，而这条漏了——没有它，**任何玩家**都能把
        // 64 格内任意一只女仆开到任意坐标（服务器上就是"我家的女仆被人遥控走了"）。
        dispatcher.register(Commands.literal("maid_smart")
                .requires(src -> src.hasPermission(2)) // 仅 OP（与另两条命令同口径）
