...
public class MaidSlaughterBehavior extends Behavior<EntityMaid> {
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        if (this.target != null
                && this.target.distanceToSqr(maid) > this.scanRadiusSq(maid) * 1.44) {
            this.target = null; // 追出圈：放弃本目标重新索敌（1.2 倍半径容差）
        }
        // 近身宰杀：距目标 ≤ 3 格 → kill + 挥臂 + 冷却
        if (this.target != null) {
            double distSq = this.target.distanceToSqr(maid);
            if (distSq <= STRIKE_DIST * STRIKE_DIST) {
                ...
                com.maidsmart.tool.PromaidLog.log("宰杀",
                        com.maidsmart.tool.PromaidLog.nameOf(maid) + " 宰杀 "
                                ...
                                .getKey(this.target.getType()));
                this.target.kill(); // kill = Entity.kill——正常死亡流程，掉落物照常
                maid.swing(net.minecraft.world.InteractionHand.MAIN_HAND); // 挥臂动画
                this.target = null;
                return;
            }
        ...
        if (picked != null) {
            ...
            net.minecraft.core.BlockPos spot = new net.minecraft.core.BlockPos(
                    base.getX() + dx, base.getY(), base.getZ() + dz);
    ...
    private Animal scanAndPick(ServerLevel level, EntityMaid maid) {
        ...
        for (net.minecraft.world.entity.Entity e : level.getEntitiesOfClass(
            ...
            if (a.getType() == net.minecraft.world.entity.EntityType.SKELETON_HORSE
                    || a.getType() == net.minecraft.world.entity.EntityType.ZOMBIE_HORSE) {
                continue; // 亡灵马（1.21.1 无 MobType，按实体类型排除）
            }
            // 实测五百六十二：home 模式限制圈外的牲畜不进组（工作区域=工位锚点圈，
            // 追出圈的追踪走位会被原版 Await 清掉、收工还会被拉回——不如一开始不选）
            if (!com.maidsmart.follow.WorkAreaClamp.allows(maid, a.blockPosition())) {
                continue;
            }
            groups.computeIfAbsent(a.getType(), k -> new ArrayList<>()).add(a);
        }
        ...
        if (groups.isEmpty()) {
            com.maidsmart.tool.PromaidLog.log("宰杀",
                    com.maidsmart.tool.PromaidLog.nameOf(maid)
                            ...
                            + "，位于 " + maid.getX() + "," + maid.getY() + "," + maid.getZ() + "）");
            ...
            if (this.diagCooldown-- <= 0) {
                ...
                for (net.minecraft.world.entity.Entity e : level.getEntitiesOfClass(
                    ...
                    double dx = e.getX() - maid.getX();
                    double dz = e.getZ() - maid.getZ();
                    all.append(ek == null ? "?" : ek.getPath())
                            .append('@').append((int) e.getX()).append(',').append((int) e.getY())
                            .append(',').append((int) e.getZ())
        ...
        for (Map.Entry<EntityType<?>, List<Animal>> e : groups.entrySet()) {
            ...
            net.minecraft.resources.ResourceLocation key =
                    net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE.getKey(e.getKey());
            sb.append(key == null ? "?" : key.getPath()).append("=").append(e.getValue().size());
        }
        com.maidsmart.tool.PromaidLog.log("宰杀",
        ...
        for (Map.Entry<EntityType<?>, List<Animal>> e : groups.entrySet()) {
            if (e.getValue().size() > threshold) {
                ...
                // nextInt(int) = nextInt(bound)——随机选一只
                return list.get(maid.getRandom().nextInt(list.size()));
            }
        }
        com.maidsmart.tool.PromaidLog.log("宰杀",
