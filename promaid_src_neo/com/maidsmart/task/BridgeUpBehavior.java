...
public class BridgeUpBehavior extends Behavior<EntityMaid> {
    ...
    public static EntityMaid findNearestMaidPublic(ServerLevel level, BlockPos pos) {
        ...
        for (EntityMaid m : level.getEntitiesOfClass(EntityMaid.class, new AABB(pos).inflate(8.0))) {
            if (!m.isAlive()) {
                continue;
            }
            double dSq = m.distanceToSqr(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5);
            if (dSq < bestSq) {
                bestSq = dSq;
                best = m;
            }
     ...
     *
     * 坐下的两种形态都要拦：① {@code isPassenger()}——骑乘/坐在 TLM 的椅子上（passenger 形态，
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, EntityMaid maid) {
        ...
        if (((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().getBoolean(com.maidsmart.combat.SelfPreservationBehavior.PRESERVE_TAG)) {
            return false; // 自保优先
        }
        if (isSitting(maid)) {
        ...
        int dy = owner.blockPosition().getY() - maid.blockPosition().getY();
        ...
        double hx = owner.getX() - maid.getX();
        double hy = owner.getY() - maid.getY();
        double hz = owner.getZ() - maid.getZ();
        ...
        if (maid.distanceToSqr(owner.getX(), owner.getY(), owner.getZ())
                <= minStart * minStart) {
    ...
    protected void start(ServerLevel level, EntityMaid maid, long gameTime) {
        ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().putBoolean(BRIDGING_TAG, true);
        this.stepCooldown = 0;
        this.guardTicks = 0;
        ...
        com.mojang.logging.LogUtils.getLogger().info(
                ...
                maid.getOwner() != null
                        ? maid.getOwner().blockPosition().getY() - maid.blockPosition().getY() : -999);
    }
    ...
    private String stopReason(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        int dyNow = owner.blockPosition().getY() - maid.blockPosition().getY();
        double hxS = owner.getX() - maid.getX();
        double hzS = owner.getZ() - maid.getZ();
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        double distSq = maid.distanceToSqr(owner.getX(), owner.getY(), owner.getZ());
        int dy = owner.blockPosition().getY() - maid.blockPosition().getY();
        // 完成：贴到主人身边（跟随接管）
        if (distSq <= 6.25) {
        ...
        if (this.walkOnTicks > 0) {
            ...
            if (arrived) {
                ...
                double wdx = this.walkOnX - maid.getX();
                double wdz = this.walkOnZ - maid.getZ();
                ...
                if (wd > 1e-3) {
                    ...
                    net.minecraft.world.level.Level wl = maid.level();
                    boolean wGuard = true;
                    try {
                        wGuard = !wl.getBlockState(wcell).getCollisionShape(wl, wcell,
                                net.minecraft.world.phys.shapes.CollisionContext.empty()).isEmpty()
                                || wl.getBlockState(wcell.below()).getCollisionShape(wl, wcell.below(),
                                net.minecraft.world.phys.shapes.CollisionContext.empty()).isEmpty();
                    } catch (Exception ignored) {
                        wGuard = true;
                    }
                    ...
                    double wvy = this.walkOnY > maid.getY() + 0.5
        ...
        if (!diag && (hDist > 1.2 || dy < 0)) {
            maid.getNavigation().moveTo(owner.getX(), maid.getY(), owner.getZ(),
                    (float) (double) MaidSmartConfig.MINE_MOVE_SPEED.get());
        } else if (!diag) {
            maid.getNavigation().stop(); // 站桩搭高（垂直列干净成型）
        }
    ...
    private boolean tryAirBridgeStep(ServerLevel level, EntityMaid maid, double hx, double hz, double hDist) {
        ...
        for (double[] d : dirs) {
            ...
            if (!level.getBlockState(ahead).isAir()
                    || !level.getBlockState(ahead.offset(0, 1, 0)).isAir()) {
                continue;
            }
    ...
    private boolean tryDiagStep(ServerLevel level, EntityMaid maid, double hx, double hz, double hDist) {
        ...
        for (double[] d : dirs) {
            ...
            if (level.getBlockState(fill).isAir()) {
                place = fill;
            } else if (level.getBlockState(fill).isRedstoneConductor(level, fill)) {
                place = ahead;
            } else {
                continue;
            }
    ...
    protected boolean canStillUse(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        if (maid.distanceToSqr(owner.getX(), owner.getY(), owner.getZ()) <= 6.25) {
        ...
        int dyNow = owner.blockPosition().getY() - maid.blockPosition().getY();
        double hxC = owner.getX() - maid.getX();
        double hzC = owner.getZ() - maid.getZ();
        ...
        if (!gapAheadC && distLimit != Integer.MAX_VALUE
                && maid.distanceToSqr(owner.getX(), owner.getY(), owner.getZ())
                >= sq(distLimit + 2)) {
    ...
    protected void stop(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        com.mojang.logging.LogUtils.getLogger().info(
                ...
                maid.getDisplayName() != null ? maid.getDisplayName().getString() : maid.getUUID(),
                reason);
        ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().putBoolean(BRIDGING_TAG, false);
        NO_BLOCK_SINCE.remove(maid.getId());
        ...
        if ("no-block".equals(reason) || "head-blocked".equals(reason)) {
            canUseThrottle.put(maid.getId(), 20);
        } else {
            canUseThrottle.remove(maid.getId());
        }
    ...
    private void placeStep(ServerLevel level, EntityMaid maid) {
        ...
        BlockPos place = level.getBlockState(pos).isAir() ? pos : pos.offset(0, 1, 0);
        ...
        if (!level.getBlockState(headPos).isAir() || !level.getBlockState(headPos.offset(0, 1, 0)).isAir()) {
            return;
        }
        // 实测五百三十六：目标格被主人碰撞箱占着就不搭
        // （防把主人痊住/盖头）——在取料前拦，不浪费方块
        ...
        stepOnto(maid, maid.getX(), 0, maid.getZ());
    }
     ...
     * tick 开头的 walkOn 段持续推送直到踏入目标格（踏入即停 / 12 tick 超时）。
    ...
    private static void stepOnto(EntityMaid maid, double tx, double ty, double tz) {
        ...
        if (d > 1e-3) {
            ...
            maid.setDeltaMovement(new net.minecraft.world.phys.Vec3(dx / d * v, 0.0, dz / d * v));
        } else {
            // 垂直垫块（d≈0）：原地起跳站上新方块
            net.minecraft.world.phys.Vec3 cur = maid.getDeltaMovement();
            double vy = Math.max(cur.y, 0.42);
            maid.setDeltaMovement(new net.minecraft.world.phys.Vec3(0.0, vy, 0.0));
        }
    }
    ...
    private void notifyNoBlock(EntityMaid maid) {
        ...
        try {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ...
                if (st.isEmpty() || !(st.getItem() instanceof net.minecraft.world.item.BlockItem bi)) {
                    continue;
                }
    ...
    private void antiSuffocate(EntityMaid maid) {
        ...
        var st = maid.level().getBlockState(mid);
    ...
    private static boolean hasThreatNearby(ServerLevel level, EntityMaid maid) {
        ...
        for (net.minecraft.world.entity.Entity e : level.getEntitiesOfClass(
            ...
            if (e.isAlive() && e instanceof net.minecraft.world.entity.monster.Enemy) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isTaskOccupied(EntityMaid maid) {
        ...
        try {
            var target = maid.getBrain().getMemory(
        ...
        return ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().getBoolean(MaidWorkTags.WORK_STILL_TAG);
    }
    ...
    private static boolean hasGapAhead(ServerLevel level, EntityMaid maid,
        ...
        for (int step = 1; step <= 2; step++) {
            int tx = (int) Math.floor(maid.getX() + ux * step);
            int tz = (int) Math.floor(maid.getZ() + uz * step);
            if (!level.getBlockState(new BlockPos(tx, y, tz)).isAir()) {
                continue; // 前方有实体方块挡（小丘/台阶——导航会绕/跳上，不算悬空）
            }
            if (!level.getBlockState(new BlockPos(tx, y + 1, tz)).isAir()) {
                continue; // 顶头——绕，不算悬空
            }
            if (level.getBlockState(new BlockPos(tx, y - 1, tz)).isAir()) {
