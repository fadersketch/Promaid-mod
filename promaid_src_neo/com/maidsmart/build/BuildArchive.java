...
public class BuildArchive extends net.minecraft.world.level.saveddata.SavedData {
    ...
    public static BuildArchive load(net.minecraft.nbt.CompoundTag tag) {
        ...
        a.deserializePlans(tag.contains("plans", 8) ? tag.getString("plans") : "");
        // 旧档迁移：单计划字段 → 第一个 SavedPlan（legacy: 前缀 planId 保持稳定）
        if (a.plans.isEmpty() && tag.contains("blueprintId", 8)) {
            String oldId = tag.getString("blueprintId");
            if (!oldId.isEmpty()) {
                SavedPlan p = new SavedPlan();
                p.planId = "legacy:" + tag.getInt("ox") + "," + tag.getInt("oy") + "," + tag.getInt("oz");
                p.ox = tag.getInt("ox");
                p.oy = tag.getInt("oy");
                p.oz = tag.getInt("oz");
                p.blueprintId = oldId;
                p.name = tag.getString("name");
                p.cursor = tag.getInt("cursor");
                // v1.5.250：contains 是 containsKey 不是 getBoolean。
                // v1.2.0 实测四百八十三【修】：getTagType 返回类型 id（ByteTag 恒为 1 →
                // `!= 0` 恒真，paused 读不回 false）；读值要用 getByte。
                p.paused = tag.getByte("paused") != 0;
                p.foremanUuid = tag.contains("foremanUuid", 8) ? tag.getString("foremanUuid") : "";
                a.plans.add(p);
            }
        }
        return a;
    ...
    public net.minecraft.nbt.CompoundTag save(net.minecraft.nbt.CompoundTag tag,
        ...
        tag.putString("plans", serializePlans());
        return tag;
    }

    /** 取当前维度的建造档案（懒加载，自动建档） */
    public static BuildArchive get(net.minecraft.server.level.ServerLevel level) {
        return level.getDataStorage().computeIfAbsent(new net.minecraft.world.level.saveddata.SavedData.Factory<BuildArchive>(BuildArchive::new,
                        (t, registries) -> BuildArchive.load(t),
