...
public class SmartBuildTool implements ITool<SmartBuildTool.Result> {
    ...
    public LLMCallback onCall(String toolId, Result result, LLMCallback callback) {
        ...
        List<String> steps = BlueprintLib.getBlueprintRotated(blueprint, quarters,
                maid.level().holderLookup(net.minecraft.core.registries.Registries.BLOCK));
        ...
        if (boundId != null) {
            ...
            try {
                ...
                if (exPs != null) {
                    where = "（已绑定「" + exPs.name + "」在 " + exPs.origin.getX() + ","
                            + exPs.origin.getY() + "," + exPs.origin.getZ()
                            + "——走到该区块内即可看到手册控制按钮）";
                }
            } catch (Exception ignored) {
            }
        ...
        List<String> pending = BlueprintLib.filterBuilt(maid.level(), origin, centered);
        if (pending.isEmpty()) {
        ...
        for (int i = 1; i < plan.size(); i++) {
            ...
            try {
                ...
                net.minecraft.world.level.block.Block block = net.minecraft.core.registries.BuiltInRegistries.BLOCK
                        .get(net.minecraft.resources.ResourceLocation.parse(parts[3]));
                BlockPos target = origin.offset(x, y, z);
                net.minecraft.world.level.block.state.BlockState state = maid.level().getBlockState(target);
                if (block != null && (state.getBlock() == block || BlueprintLib.isBuiltEquivalent(parts[3], state.getBlock()))) {
                    done++;
                } else if (block != null) {
    ...
    private LLMCallback startBuild(EntityMaid maid, List<String> steps, String name, String notice,
        ...
        if (overlap != null) {
            return callback.addToolResult("这个位置与已有区块「" + overlap.name + "」（"
                    ...
                    + "," + overlap.origin.getZ() + "）重叠了。请换个位置再试。", toolId);
        }
        ...
        try {
            maid.getBrain().setActiveActivityIfPossible(net.minecraft.world.entity.schedule.Activity.WORK);
        } catch (Exception ignored) {
        }
