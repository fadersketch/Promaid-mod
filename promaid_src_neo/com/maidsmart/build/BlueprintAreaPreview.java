...
public final class BlueprintAreaPreview {
    ...
    public static void resetOriginToPlayer() {
        ...
        if (mc.player != null) {
            previewOrigin = mc.player.blockPosition();
        }
    }
    ...
    private static void ensureRegistered() {
        if (!registered) {
            registered = true;
            net.neoforged.neoforge.common.NeoForge.EVENT_BUS.register(BlueprintAreaPreview.class);
        }
    }
    ...
    public static void onRender(net.neoforged.neoforge.client.event.RenderLevelStageEvent event) {
        ...
        net.neoforged.neoforge.client.event.RenderLevelStageEvent.Stage wantStage =
        ...
        net.minecraft.world.phys.Vec3 camera = event.getCamera().getPosition().reverse();
        ...
        if (active) {
            ...
            net.minecraft.core.BlockPos p = previewOrigin != null ? previewOrigin : mc.player.blockPosition();
            double x0 = p.getX() - effX / 2.0;
            double z0 = p.getZ() - effZ / 2.0;
            double y0 = p.getY();
            double x1 = x0 + effX;
            double z1 = z0 + effZ;
            double y1 = y0 + sizeY;
    ...
    public static void onClientTick(net.neoforged.neoforge.client.event.ClientTickEvent.Post event) {
        while (com.maidsmart.build.BuildKeysClient.ROTATE_BLUEPRINT != null
                && com.maidsmart.build.BuildKeysClient.ROTATE_BLUEPRINT.consumeClick()) {
            if (active) {
                rotateClockwise();
            }
        }
        ...
        while (com.maidsmart.build.BuildKeysClient.PLACEMENT_SCREEN != null
                && com.maidsmart.build.BuildKeysClient.PLACEMENT_SCREEN.consumeClick()) {
            if (active && previewId != null
     ...
     * 渲染本体 = DebugRenderer.renderFilledBox（renderFilledBox，vanilla debug 管线，长期
    ...
    private static void drawGhost(com.mojang.blaze3d.vertex.PoseStack pose,
        ...
        if (com.maidsmart.config.MaidSmartConfig.BUILD_REAL_GHOST_BLOCKS.get()) {
            ...
            if (states != null) {
                drawRealGhost(pose, mc, camera, pts, states, ox, oy, oz);
                return;
            }
        }
        ...
        if (total > 0) {
            ...
            for (int i = 0; i < total; i++) {
                double dx = ox + (int) pts[i * 4] + 0.5 - camera.x;
                double dy = oy + (int) pts[i * 4 + 1] + 0.5 - camera.y;
                double dz = oz + (int) pts[i * 4 + 2] + 0.5 - camera.z;
        ...
        for (int i = 0; i + 3 < pts.length; i += 4) {
            ...
            if (fillSel != null && fillSel[i / 4]) {
                net.minecraft.world.phys.AABB box = new net.minecraft.world.phys.AABB(
                        wx, wy, wz, wx + 1.0, wy + 1.0, wz + 1.0).move(camera);
                net.minecraft.client.renderer.debug.DebugRenderer.renderFilledBox(
                        pose, ghostSource, box, r, g, b, a);
                ghostSource.endBatch(net.minecraft.client.renderer.RenderType.debugFilledBox());
                fillCount++;
            }
            drawn++;
        }
        ...
        for (int i = 0; i < pal.length; i++) {
            ...
            try {
                ...
                net.minecraft.commands.arguments.blocks.BlockStateParser.BlockResult res =
                        net.minecraft.commands.arguments.blocks.BlockStateParser.parseForBlock(lookup, text, false);
                states[i] = res.blockState();
                ok++;
    ...
    private static void drawRealGhost(com.mojang.blaze3d.vertex.PoseStack pose,
        ...
        net.minecraft.client.renderer.block.BlockRenderDispatcher brd = mc.getBlockRenderer();
        ...
        for (int k = 0; k < limit; k++) {
            ...
            if (level.getBlockState(bp).is(st.getBlock())) {
                continue;
            }
            // 本帧的 PoseStack 没有做 -camera 平移（描边/填充盒都是手算相机相对坐标），
            // 所以这里自己平移到"相机相对位置"，BlockPos 仍用世界坐标算光照/种子。
            pose.pushPose();
            pose.translate((float) (bp.getX() - camera.x),
                    (float) (bp.getY() - camera.y),
                    (float) (bp.getZ() - camera.z));
            brd.getModelRenderer().tesselateBlock(level, brd.getBlockModel(st), st, bp, pose, vc, false,
                    level.random, st.getSeed(bp),
                    net.minecraft.client.renderer.texture.OverlayTexture.NO_OVERLAY,
                    net.neoforged.neoforge.client.model.data.ModelData.EMPTY, null);
            pose.popPose();
            drawn++;
        }
        buffers.endBatch(net.minecraft.client.renderer.Sheets.translucentCullBlockSheet());
        if (drawn != lastDrawnCount) {
            lastDrawnCount = drawn;
     ...
     * （1.21.1 的 VertexConsumer 是 6 个抽象方法的 Mojmap 版，javap 实证）
     */
    private static final class GhostBlockVertexConsumer
        ...
        public com.mojang.blaze3d.vertex.VertexConsumer setColor(int r, int g, int b, int a) {
            delegate.setColor(dim(r), dim(g), dim(b), (int) (a * 0.6f));
            return this;
        }
    ...
    private static void drawBoxEdges(com.mojang.blaze3d.vertex.PoseStack pose,
        ...
        net.minecraft.world.phys.Vec3 c0 = camera.add(x0, y0, z0);
        net.minecraft.world.phys.Vec3 c1 = camera.add(x1, y0, z0);
        net.minecraft.world.phys.Vec3 c2 = camera.add(x1, y0, z1);
        net.minecraft.world.phys.Vec3 c3 = camera.add(x0, y0, z1);
        net.minecraft.world.phys.Vec3 t0 = camera.add(x0, y1, z0);
        net.minecraft.world.phys.Vec3 t1 = camera.add(x1, y1, z0);
        net.minecraft.world.phys.Vec3 t2 = camera.add(x1, y1, z1);
        net.minecraft.world.phys.Vec3 t3 = camera.add(x0, y1, z1);
