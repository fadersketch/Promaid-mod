...
public final class WorldProbe {
    ...
    private static ServerLevel level(EntityMaid maid) {
        return maid.level() instanceof ServerLevel sl ? sl : null;
    }
    ...
    private static char classifyColumn(ServerLevel lv, int x, int feetY, int z) {
        BlockState feet = lv.getBlockState(new BlockPos(x, feetY, z));
        BlockState head = lv.getBlockState(new BlockPos(x, feetY + 1, z));
    ...
    private static boolean canStandAt(ServerLevel lv, int x, int y, int z) {
        BlockState below = lv.getBlockState(new BlockPos(x, y - 1, z));
        BlockState body = lv.getBlockState(new BlockPos(x, y, z));
        BlockState head = lv.getBlockState(new BlockPos(x, y + 1, z));
    ...
    private static boolean fullyPassable(ServerLevel lv, int x, int y, int z) {
        BlockState state = lv.getBlockState(new BlockPos(x, y, z));
        return state.getFluidState().isEmpty() && state.canOcclude();
    }
    ...
    private static boolean isLiquid(BlockState state) {
        return !state.getFluidState().isEmpty() && !isLava(state);
    }
        ...
        for (int dx = 0; dx < width; dx++) {
            for (int dz = 0; dz < depth; dz++) {
                ...
                for (int y = base + 1; y <= base + 3; y++) {
                    ...
                    if (blockPath(state.getBlock()).endsWith("_log")
                            || blockPath(state.getBlock()).endsWith("_stem")
                            || blockPath(state.getBlock()).endsWith("_leaves")) {
                    ...
                    obstacles.add(new Obstacle(pos, "WALL", "障碍方块 " + blockPath(state.getBlock())));
                    clearCount++;
                }
    ...
    public static String formatSiteReport(SiteInfo a) {
        ...
        if (!a.obstacles().isEmpty()) {
            ...
            for (Obstacle o : a.obstacles()) {
                ...
                sb.append("    [").append(o.type()).append("] (").append(o.pos().getX()).append(",")
                        .append(o.pos().getY()).append(",").append(o.pos().getZ()).append(") ")
    ...
    public static String inspectBlock(EntityMaid maid, int x, int y, int z) {
        ...
        ResourceLocation id = net.minecraft.core.registries.BuiltInRegistries.BLOCK.getKey(block);
        // getDestroySpeed=getDestroySpeed（BlockBehaviour 反编译实证）；掉落工具需求无公开 getter，
        // 用硬度推断（基岩/黑曜石硬度 <0 不可破坏，其余需要工具的方块硬度较高）
        float hardness = state.getDestroySpeed(lv, pos);
        ...
        sb.append("  状态: ").append(state.isAir() ? "空气" : blockPath(state.getBlock())).append("\n");
        return sb.toString();
    }
    ...
    public static String scanBlock(EntityMaid maid, String blockName, int radius) {
        ...
        for (int dx = -r; dx <= r; dx++) {
            for (int dy = -r; dy <= r; dy++) {
                for (int dz = -r; dz <= r; dz++) {
                    ...
                    if (lv.getBlockState(pos).getBlock() != target) {
                        continue;
                    }
                    double dist = Math.sqrt(Math.pow(pos.getX() - ox, 2)
                            + Math.pow(pos.getY() - oy, 2)
                            + Math.pow(pos.getZ() - oz, 2));
        ...
        for (int i = 0; i < limit; i++) {
            ...
            sb.append("    at ").append(p.getX()).append(", ").append(p.getY()).append(", ")
                    .append(p.getZ()).append(" dist=").append(String.format("%.1f", (Double) found.get(i)[1])).append("格\n");
        }
        return sb.toString();
    }
    ...
    private static String entityId(LivingEntity e) {
        try {
            net.minecraft.resources.ResourceLocation key = net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE.getKey(e.getType());
    ...
    public static String scanEntities(EntityMaid maid, String type, int radius) {
        ...
        if (wantHostile || wantPassive) {
            for (Mob m : lv.getEntitiesOfClass(Mob.class, box, e -> true)) {
                double dist = m.position().distanceTo(maid.position());
                if (dist > r) {
                    continue;
                }
                boolean hostile = isHostile(m);
                if (hostile && wantHostile) {
                    entries.add(new Object[]{"[敌对]", entityId(m), m.blockPosition(), m.getHealth(), m.getMaxHealth(), dist});
                } else if (!hostile && wantPassive) {
                    entries.add(new Object[]{"[中立/被动]", entityId(m), m.blockPosition(), m.getHealth(), m.getMaxHealth(), dist});
                }
            }
        }
        if (wantPlayer) {
            for (Player p : lv.getEntitiesOfClass(Player.class, box, e -> true)) {
                if (p.getUUID().equals(maid.getUUID())) {
                    continue;
                }
                double dist = p.position().distanceTo(maid.position());
                if (dist <= r) {
                    entries.add(new Object[]{"[玩家]", p.getDisplayName() != null ? p.getDisplayName().getString() : "?", p.blockPosition(), p.getHealth(), p.getMaxHealth(), dist});
                }
            }
        }
        if (entries.isEmpty()) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            sb.append("  ").append(e[0]).append(" ").append(e[1])
                    .append(" at ").append(p.getX()).append(",").append(p.getY()).append(",").append(p.getZ())
    ...
    public static int countThreats(EntityMaid maid, double radius) {
        ...
        int n = 0;
        for (Mob m : lv.getEntitiesOfClass(Mob.class, maid.getBoundingBox().inflate(radius), e -> true)) {
