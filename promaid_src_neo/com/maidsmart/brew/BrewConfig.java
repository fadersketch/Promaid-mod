...
public final class BrewConfig {
    ...
    public static BrewConfig load(EntityMaid maid) {
        ...
        try {
            CompoundTag tag = ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().getCompound(TAG);
            if (tag == null) {
                return cfg;
            }
            cfg.mode = clamp(tag.getInt("mode"), MODE_BATCH, MODE_TARGETED, MODE_BATCH);
            cfg.enhance = clamp(tag.getInt("enhance"), ENHANCE_NONE, ENHANCE_GLOWSTONE, ENHANCE_NONE);
            cfg.form = clamp(tag.getInt("form"), FORM_DRINK, FORM_LINGERING, FORM_DRINK);
            cfg.targetPotion = tag.getString("targetPotion");
    ...
    public static void save(EntityMaid maid, BrewConfig cfg) {
        try {
            ...
            tag.putInt("mode", cfg.mode);
            tag.putInt("enhance", cfg.enhance);
            tag.putInt("form", cfg.form);
            tag.putString("targetPotion", cfg.targetPotion == null ? "" : cfg.targetPotion);
            ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().put(TAG, tag);
        } catch (Throwable ignored) {
        }
    }
    ...
    public boolean hasValidTarget() {
        ...
        try {
            return net.minecraft.core.registries.BuiltInRegistries.POTION
                    .get(ResourceLocation.parse(targetPotion)) != null;
        } catch (Throwable t) {
            return false;
        }
    }
