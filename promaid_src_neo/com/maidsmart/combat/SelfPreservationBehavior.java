...
public class SelfPreservationBehavior extends Behavior<EntityMaid> {
    ...
    public static boolean isSelfPreserving(EntityMaid maid) {
        try {
            return ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().getBoolean(PRESERVE_TAG);
    ...
    private static void destroyCombatMarked(ServerLevel level, BlockPos pos, String blockId) {
        ...
        net.minecraft.resources.ResourceLocation cur = net.minecraft.core.registries.BuiltInRegistries.BLOCK.getKey(state.getBlock());
        ...
        if (com.maidsmart.config.MaidSmartConfig.BRIDGE_RECLAIM_TO_MAID.get()) {
            java.util.List<net.minecraft.world.item.ItemStack> drops =
                    net.minecraft.world.level.block.Block.getDrops(state, level, pos, null);
            EntityMaid nearest = com.maidsmart.task.BridgeUpBehavior.findNearestMaidPublic(level, pos);
            boolean handed = false;
            if (nearest != null && !drops.isEmpty()) {
                try {
                    net.neoforged.neoforge.items.wrapper.CombinedInvWrapper inv = nearest.getAvailableInv(true);
                    for (net.minecraft.world.item.ItemStack stack : drops) {
                        if (stack.isEmpty()) {
                            continue;
                        }
                        net.minecraft.world.item.ItemStack remain = net.neoforged.neoforge.items.ItemHandlerHelper
                                .insertItemStacked(inv, stack, false);
                        if (!remain.isEmpty()) {
                            net.minecraft.world.level.block.Block.popResource(level, pos, remain);
                        }
                    }
                    handed = true;
     ...
     * 也检测不到威胁，自保不触发。这里用 LivingIncomingDamageEvent 记录 5 秒内的攻击者，
    ...
    public static void onMaidHurt(net.neoforged.neoforge.event.entity.living.LivingIncomingDamageEvent event) {
        ...
        // LivingIncomingDamageEvent，坐着被烫会立即起身逃生
        ...
        try {
            ...
            if (src0 != null) {
                String type = src0.getMsgId();
                String extra = "";
                try {
                    if (src0.scalesWithDifficulty()) {
                        extra = " [弹射物]";
                    }
                } catch (Throwable ignored) {
                }
                net.minecraft.world.entity.Entity srcEnt = src0.getDirectEntity();
                String srcName = srcEnt == null ? "无来源实体"
                        : (srcEnt.getDisplayName() != null && !srcEnt.getDisplayName().getString().isEmpty()
                                ? srcEnt.getDisplayName().getString()
                ...
                com.mojang.logging.LogUtils.getLogger().info(String.format(
                        ...
                        maid.getDisplayName() != null && !maid.getDisplayName().getString().isEmpty()
                                ? maid.getDisplayName().getString() : maid.getUUID().toString(),
                        type, extra, srcName,
                        event.getAmount(),
        ...
        if (src == null || !(src.getEntity() instanceof LivingEntity attacker)) {
        ...
        long now = maid.level().getGameTime();
        LAST_ATTACKERS.put(maid.getUUID(),
                new AttackerRecord(attacker.getId(), now, maid.level().dimension()));
    ...
    public static LivingEntity recentAttacker(EntityMaid maid) {
        ...
        long now = maid.level().getGameTime();
        if (now - rec.gameTick() >= 100L || !rec.dim().equals(maid.level().dimension())) {
            return null;
        }
        net.minecraft.world.entity.Entity e = ((ServerLevel) maid.level()).getEntity(rec.entityId());
        if (e instanceof LivingEntity lv && lv.isAlive()) {
            return lv;
        }
        return null;
    }
    ...
    private static boolean goldenAppleReady(EntityMaid maid) {
        try {
            long now = maid.level().getGameTime();
            return maid.getPersistentData().getLong(GOLDEN_APPLE_CD_TAG) <= now;
        } catch (Throwable ignored) {
            return true;
        }
    }
    ...
    private static void markGoldenAppleUsed(EntityMaid maid, boolean enchanted) {
        try {
            long now = maid.level().getGameTime();
            maid.getPersistentData().putLong(GOLDEN_APPLE_CD_TAG, now + (enchanted ? 6000L : 2400L));
        } catch (Throwable ignored) {
        }
    }
    ...
    public static long goldenAppleCdTicksLeft(EntityMaid maid) {
        try {
            long until = maid.getPersistentData().getLong(GOLDEN_APPLE_CD_TAG);
            return Math.max(0L, until - maid.level().getGameTime());
        } catch (Throwable ignored) {
            return 0L;
        }
    }
    ...
    private static String potionKey(ItemStack stack) {
        try {
            net.minecraft.world.item.alchemy.Potion potion =
                    com.maidsmart.brew.PotionCompat.of(stack);
            if (potion == null) {
                return "unknown";
            }
            net.minecraft.resources.ResourceLocation key = net.minecraft.core.registries.BuiltInRegistries.POTION.getKey(potion);
            if (key == null) {
                return "unknown";
            }
    ...
    /** v1.5.209：连续处于岩浆内的 tick 数（tick 每 tick 维护）——"安静走出"
     ...
     *  的上升沿进入，血 ≥70% 且环境安全后结束）——会话进出由 tick 维护，
     *  行为本身常驻（canUse 只查开关） */
    ...
    public static boolean isMovingToSurvive(EntityMaid maid) {
        return MOVING_SURVIVE.contains(maid.getUUID());
    }
    ...
    private static float hpRatio(EntityMaid maid) {
        return maid.getHealth() / Math.max(1.0f, maid.getMaxHealth());
    }
    ...
    private boolean envDangerCritical(EntityMaid maid) {
        return maid.isInLava() && !hasFireResist(maid)
    ...
    private boolean envDangerSoft(EntityMaid maid) {
        ...
        return (maid.isOnFire() && !hasFireResist(maid) && !fireProtected)   // isOnFire 且无抗火且无火焰保护饰品
                || (maid.isInWater() && maid.getAirSupply() < 60 && !drownProtected) // isInWater 且氧气不足 1/5 且无溺水保护饰品
    ...
    protected void start(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        // tick 的 sessionEnter（低血/环境危险的上升沿）执行
    }
    ...
    private void sessionEnter(EntityMaid maid) {
        ...
        maid.getBrain().eraseMemory(MemoryModuleType.WALK_TARGET); // 清 WALK_TARGET 残留
        ...
        net.minecraft.world.item.Item handItem = maid.getMainHandItem().getItem();
        String weaponName = handItem == null ? "empty"
                : net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(handItem).toString();
    ...
    private void tryEquipBaubleTotem(EntityMaid maid) {
        try {
            ...
            for (int i = 0; i < bauble.getSlots(); i++) {
                if (bauble.getStackInSlot(i).is(net.minecraft.world.item.Items.TOTEM_OF_UNDYING)) {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ...
                if (s.isEmpty() || !s.is(net.minecraft.world.item.Items.TOTEM_OF_UNDYING)) {
                    continue;
                }
     ...
     * 近战/三叉戟 → 直接挥砍（doHurtTarget，三叉戟戳有 7 点伤害，横扫走 TLM 逻辑）。
    ...
    private boolean goheiDanmaku(EntityMaid maid, LivingEntity threat) {
        try {
            ...
            net.minecraft.world.entity.ai.attributes.AttributeInstance ai =
                    maid.getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE);
            if (ai != null) {
                atk = (float) ai.getBaseValue();
            }
            float damage = atk * 2.2f;
            float velocity = 0.6f + net.minecraft.util.Mth.clamp(
                    (float) (dist / 40.0 - 0.4), 0.0f, 2.4f);
            float inaccuracy = (1.0f - net.minecraft.util.Mth.clamp(
    ...
    private boolean shootPointBlank(EntityMaid maid, LivingEntity threat) {
        try {
            ...
            if (arrowStack.isEmpty() || !(arrowStack.getItem() instanceof net.minecraft.world.item.ArrowItem ai)) {
                return false; // 背包没箭 → 不射（继续逃跑）
            }
            net.minecraft.world.level.Level level = maid.level();
            net.minecraft.world.entity.projectile.AbstractArrow arrow = ai.createArrow(level, arrowStack, maid, maid.getMainHandItem());
            arrow.setOwner(maid); // setOwner
            // 瞄准威胁胸口（原版 Mob 射击同款：目标中心 + 高度 30%）
            double dx = threat.getX() - maid.getX();
            double dy = threat.getY() + threat.getBbHeight() * 0.3
                    - (maid.getY() + maid.getBbHeight() * 0.5);
            double dz = threat.getZ() - maid.getZ();
            double dist = Math.sqrt(dx * dx + dz * dz);
            arrow.shoot(dx, dy + dist * 0.08, dz, 1.6f, 1.0f); // shoot（1.6 速，贴脸必中）
            level.addFreshEntity(arrow);
            maid.swing(net.minecraft.world.InteractionHand.MAIN_HAND, true); // 挥臂
            return true;
        } catch (Exception ignored) {
            return false;
        }
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        // 播报保护用）
        this.walkOutTimer = maid.isInLava() ? this.walkOutTimer + 1 : 0;
        if (maid.isInLava()) {
            this.lastInLavaTick = maid.level().getGameTime();
        }
        ...
        if (!this.sessionActive) {
            ...
            if (((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().getBoolean(PRESERVE_TAG) && !danger && ratio >= enterRatio()) {
                ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().putBoolean(PRESERVE_TAG, false);
                MOVING_SURVIVE.remove(maid.getUUID());
        ...
        if (!this.loggedDecision && threat != null) {
            ...
            com.maidsmart.tool.PromaidLog.log("自保诊断",
                    com.maidsmart.tool.PromaidLog.nameOf(maid)
                            ...
                            + " 距威胁 " + String.format(java.util.Locale.ROOT, "%.1f", maid.distanceTo(threat)) + " 格"
        ...
        boolean atSafeHeight = this.pillarBaseY >= 0 && !this.forcedPillar
                && maid.blockPosition().getY() - this.pillarBaseY >= safePillarHeight();
        ...
        if (pillaring) {
            ...
            if (!this.pillarLocked) {
                this.pillarLockX = Math.floor(maid.getX()) + 0.5;
                this.pillarLockZ = Math.floor(maid.getZ()) + 0.5;
                this.pillarLocked = true;
            }
            double dxc = maid.getX() - this.pillarLockX;
            double dzc = maid.getZ() - this.pillarLockZ;
            if (dxc * dxc + dzc * dzc <= 1.0) {
                maid.setPos(this.pillarLockX, maid.getY(), this.pillarLockZ);
            } else {
            ...
            if (this.walkOnTicks > 0) {
                this.walkOnTicks--;
                if (maid.getY() >= this.walkOnY - 0.05) {
                    this.walkOnTicks = 0; // 站上目标格
                } else {
                    net.minecraft.world.phys.Vec3 vw = maid.getDeltaMovement();
                    maid.setDeltaMovement(new net.minecraft.world.phys.Vec3(0.0,
                            Math.max(vw.y, 0.42), 0.0));
                }
            }
        } else {
            this.pillarLocked = false;
        ...
        boolean moving = danger || sitting || (threat != null && ratio < 0.3f)
                || pillaring;
        if (moving) {
            MOVING_SURVIVE.add(maid.getUUID());
        } else {
            MOVING_SURVIVE.remove(maid.getUUID());
        }
        ...
        if (danger) {
            long nowGt = maid.level().getGameTime();
        ...
        if (threat == null) {
            ...
            if (this.pillarBaseY >= 0) {
                int icy = maid.blockPosition().getY();
        ...
        if (this.hpRatio(maid) >= 0.3f) {
            ...
            if (this.pillarBaseY >= 0) {
                int pcy = maid.blockPosition().getY();
        ...
        // （逃跑/搭高/珍珠/围壁）之前优先处理：直接调 doHurtTarget（doHurtTarget，
        ...
        if (this.forcedPillar) {
            int fy = maid.blockPosition().getY();
        ...
        //    这件事——"包里明明有食物，却判成没有"。 */
        int currentY = maid.blockPosition().getY();
    ...
    private void strafeDisengage(EntityMaid maid, LivingEntity threat) {
        ...
        if (this.fleeDirCooldown > 0) {
            ...
            maid.getNavigation().moveTo(this.fleeTargetX,
                    maid.blockPosition().getY(), this.fleeTargetZ, fleeSpeed());
            return;
        }
        int bestDx = 0;
        int bestDz = 0;
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                BlockPos probe = maid.blockPosition().offset(dx * 3, 0, dz * 3);
                // 路径上（1~3 格）有岩浆 → 直接排除该方向
                boolean lavaPath = false;
                for (int s = 1; s <= 3; s++) {
                    BlockPos p = maid.blockPosition().offset(dx * s, 0, dz * s);
                    if (maid.level().isLoaded(p)
                            && isLavaBlock(maid.level().getBlockState(p).getBlock())) {
                        lavaPath = true;
                        break;
                    }
                }
                ...
                int monsters = maid.level().getEntitiesOfClass(Mob.class,
                        new net.minecraft.world.phys.AABB(probe).inflate(3.0),
                ...
                double dot = 0;
                if (threat != null) {
                    dot = (dx * (maid.getX() - threat.getX())
                            + dz * (maid.getZ() - threat.getZ())) / 8.0;
                }
        ...
        int tx = (int) maid.getX() + bestDx * 2;
        int tz = (int) maid.getZ() + bestDz * 2;
        ...
        maid.getNavigation().moveTo(tx, maid.blockPosition().getY(), tz, fleeSpeed());
    }
    ...
    private boolean breakBlockingCombatBlocks(EntityMaid maid, int dx, int dz) {
        ...
        try {
            ...
            for (int s = 1; s <= 2; s++) {
                int bx = (int) Math.floor(maid.getX()) + dx * s;
                int bz = (int) Math.floor(maid.getZ()) + dz * s;
                for (int dy = 0; dy <= 1; dy++) { // 脚部+头部
                    ...
                    sl.levelEvent(2001, p, net.minecraft.world.level.block.Block.getId(state));
                    if (com.maidsmart.config.MaidSmartConfig.BRIDGE_RECLAIM_TO_MAID.get()) {
                            ...
                            try {
                                ...
                                for (net.minecraft.world.item.ItemStack stack : drops) {
                                    ...
                                    net.minecraft.world.item.ItemStack remain =
                                            net.neoforged.neoforge.items.ItemHandlerHelper.insertItemStacked(inv, stack, false);
                                    if (!remain.isEmpty()) {
                                        net.minecraft.world.level.block.Block.popResource(sl, p, remain);
                                    }
                                }
                                handed = true;
                            } catch (Exception ignored) {
                        ...
                        if (!handed && !drops.isEmpty()) {
                            for (net.minecraft.world.item.ItemStack stack : drops) {
                                net.minecraft.world.level.block.Block.popResource(sl, p, stack);
                            }
                        }
                    } else {
                        net.minecraft.world.level.block.Block.dropResources(state, sl, p, sl.getBlockEntity(p));
                    }
                    ...
                    // 前有 state.isAir 检查），表内残留条目由 clearAll/到期流程自然清理
                    broke = true;
                }
            }
        } catch (Exception ignored) {
    ...
    protected boolean canStillUse(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        // 由 tick 内部维护（sessionActive + PRESERVE_TAG），非会话 tick 零干预
        return true;
    }
    ...
    private void teleportHomeOnExit(EntityMaid maid, boolean ignoreCooldown) {
        try {
            ...
            if (!(owner.level() instanceof net.minecraft.server.level.ServerLevel ownerLevel)) {
                return;
            }
            net.minecraft.core.BlockPos stand = com.maidsmart.follow.MaidChunkLoadManager
                    .findStandNear(ownerLevel,
                            new net.minecraft.core.BlockPos((int) Math.floor(owner.getX()),
    ...
    private boolean eatHealingFood(EntityMaid maid) {
        ...
        for (int i = 0; i < inv.getSlots(); i++) {
            ...
            if (!this.isHealFood(stack.getItem())) {
                continue;
            }
            ItemStack taken = inv.extractItem(i, 1, false);
            if (taken.isEmpty()) {
            ...
            if (com.maidsmart.combat.MaidMealBridge.onFeedCooldown(maid, taken)) {
                net.neoforged.neoforge.items.ItemHandlerHelper.insertItemStacked(inv, taken, false);
                continue;
            }
            // 真实进食：应用食物属性（金苹果=吸收+再生，熟食=饱食）
            // 实测五百七十九：改走**物品自己的 finishUsingItem**（普通食物与旧版 eat() 等价，
            // "吃完不消失"的遗物类不会被手搓消耗毁掉）；没被消耗就放回背包并继续找下一种
    ...
    private LivingEntity findThreat(EntityMaid maid) {
        ...
        if (target != null && target.isAlive() && maid.distanceTo(target) < threatDistance()
                && this.hasSight(maid, target)) {
            return target;
        }
        // v1.5.135：最近攻击者（LivingIncomingDamageEvent 记录）——覆盖其他 mod 的
        ...
        if (rec != null) {
            ...
            if (now - rec.gameTick() < 100L && rec.dim().equals(maid.level().dimension())) {
                net.minecraft.world.entity.Entity e = ((ServerLevel) maid.level()).getEntity(rec.entityId());
                if (e instanceof LivingEntity lv && lv.isAlive()
                        && maid.distanceTo(lv) < threatDistance() && this.hasSight(maid, lv)) {
                    return lv;
                }
                if (e == null || !e.isAlive()) {
                    LAST_ATTACKERS.remove(maid.getUUID()); // 攻击者已死/消失 → 清记录
                }
            } else if (now - rec.gameTick() >= 100L) {
                LAST_ATTACKERS.remove(maid.getUUID()); // 窗口过期 → 清记录
            }
        }
        ...
        List<Mob> mobs = maid.level().getEntitiesOfClass(Mob.class, maid.getBoundingBox().inflate(threatDistance()),
                m -> m.isAlive());
        ...
        for (Mob mob : mobs) {
            ...
            double dist = maid.distanceTo(mob);
            if (dist < minDist) {
                minDist = dist;
                nearest = mob;
            }
    ...
    private static boolean isTargetingCenter(Mob mob, LivingEntity center) {
        try {
            ...
            LivingEntity t = mob.getTarget(); // getTarget
            if (t == null) {
                return false;
            }
            if (t == center) {
    ...
    public static boolean hasSight(LivingEntity from, LivingEntity other) {
        try {
            return hasSight(from, other.position());
        } catch (Exception e) {
            return true;
        }
    }
    ...
    public static boolean hasSight(LivingEntity from, net.minecraft.world.phys.Vec3 to) {
        try {
            net.minecraft.world.level.ClipContext ctx = new net.minecraft.world.level.ClipContext(
                    from.position().add(0.0, 1.2, 0.0),
                    to.add(0.0, 1.2, 0.0),
                    net.minecraft.world.level.ClipContext.Block.COLLIDER,
                    net.minecraft.world.level.ClipContext.Fluid.NONE, from);
            net.minecraft.world.phys.BlockHitResult hit = from.level().clip(ctx);
     ...
     *  v1.5.241：改用 getKey 注册名比较——不再依赖 net.minecraft.core.registries.BuiltInRegistries.BLOCK.getValue
    ...
    private boolean hasDangerAt(net.minecraft.world.level.Level level, BlockPos pos) {
        ...
        BlockState below = level.getBlockState(pos.offset(0, -1, 0));
        return isDangerBlock(state.getBlock()) || isDangerBlock(below.getBlock());
    }
    ...
    private void edgeGuard(EntityMaid maid) {
        ...
        BlockPos under = feet.offset(0, -1, 0);
        BlockPos support = null;
        if (maid.level().getBlockState(under).isRedstoneConductor(maid.level(), under)) {
            support = under;
        } else {
            ...
            for (int dx = -1; dx <= 1 && support == null; dx += 2) {
                for (int dz = -1; dz <= 1 && support == null; dz += 2) {
                    BlockPos diag = feet.offset(dx, -1, dz);
                    if (maid.level().getBlockState(diag).isRedstoneConductor(maid.level(), diag)) {
                        support = diag;
                    }
                }
            }
        ...
        double halfW = maid.getBbWidth() / 2.0;
        double limit = Math.max(0.05, 0.5 - halfW);
        double cx = support.getX() + 0.5;
        double cz = support.getZ() + 0.5;
        double x = maid.getX();
        double z = maid.getZ();
    ...
    private boolean bodyInSolid(EntityMaid maid) {
        net.minecraft.world.phys.AABB box = maid.getBoundingBox();
        double cy = box.minY + (box.maxY - box.minY) * 0.5; // 身体中心高度（半身）
        BlockPos mid = new BlockPos((int) Math.floor(maid.getX()),
                (int) Math.floor(cy), (int) Math.floor(maid.getZ()));
        BlockState st = maid.level().getBlockState(mid);
        return !st.isAir() && st.isRedstoneConductor(maid.level(), mid);
    }
    ...
    private boolean headInSolid(EntityMaid maid) {
        net.minecraft.world.phys.AABB box = maid.getBoundingBox();
        double hy = box.minY + (box.maxY - box.minY) * 0.9; // 头部高度
        BlockPos head = new BlockPos((int) Math.floor(maid.getX()),
                (int) Math.floor(hy), (int) Math.floor(maid.getZ()));
        BlockState st = maid.level().getBlockState(head);
        return !st.isAir() && st.isRedstoneConductor(maid.level(), head);
    }

    /** v1.5.199：背包是否有水桶（仅判定不消耗——与落地水一致）。
     *  v1.5.231b：也查【主手/副手】——主人把水桶"给"女仆时可能装备在手上
     *  （实测"给了水桶但不用水灭火"的一半原因：水桶在手不在背包）
     *  v1.5.240：改用 getKey 注册名比较——不再依赖 net.minecraft.core.registries.BuiltInRegistries.ITEM.getValue
    ...
    private boolean hasWaterBucket(EntityMaid maid) {
        if (isWaterBucketItem(maid.getMainHandItem())) {
            return true;
        }
        if (isWaterBucketItem(maid.getOffhandItem())) {
            return true;
        }
        IItemHandler inv = maid.getMaidInv();
        for (int i = 0; i < inv.getSlots(); i++) {
     ...
     *  标志，canOcclude()（1 参 canBeReplaced）对岩浆/水恒为 false；旧版只认
     *  canOcclude() → 没垫方块时站在岩浆里"自身格 + 上方格都是岩浆"判定不可放 →
    ...
    private boolean canPourWaterAt(net.minecraft.world.level.Level level, BlockPos pos) {
        ...
        // v1.5.259：canOcclude（0 参）是 isSolid（有碰撞），不是 canBeReplaced！
        // 旧版 `st.isAir() || st.canOcclude()` 把实心方块当"可替换"（植物/火把
        // isSolid=false 反而判不可倒）→ 岩浆自救方向反了。非实心（空气/植物/
        // 火把位/岩浆/水）都可被水替换，直接 !isSolid 覆盖全部。
        return !st.canOcclude();
    }
    ...
    private void placeClutchWater(EntityMaid maid) {
        try {
            ...
            net.minecraft.world.level.block.Block water = net.minecraft.world.level.block.Blocks.WATER;
            ...
            for (int up : new int[]{1, 0, 2}) {
                ...
                if (this.canPourWaterAt(level, cand)) {
                    level.setBlock(cand, water.defaultBlockState(), 3);
                    this.waterPos = cand;
                    this.waterPlacedTick = level.getGameTime();
                    // v1.5.249：放水成功诊断（节流 10 秒）——确认放水真的执行了
                    long gt2 = level.getGameTime();
                    Long lastLog = WATER_LOG_CD.get(maid.getUUID().toString());
                    if (lastLog == null || gt2 - lastLog > 200) {
                        WATER_LOG_CD.put(maid.getUUID().toString(), gt2);
                        LOGGER.info("placeClutchWater ok: maid={} waterAt={}",
                                maid.getDisplayName() != null ? maid.getDisplayName().getString() : "?",
                                cand);
                    }
                    return;
                }
            ...
            LOGGER.info("placeClutchWater fail: maid={} y={} up1={} self={} up2={}",
                    maid.getDisplayName() != null ? maid.getDisplayName().getString() : "?",
                    pos.getY(),
                    this.canPourWaterAt(level, pos.offset(0, 1, 0)),
                    this.canPourWaterAt(level, pos),
                    this.canPourWaterAt(level, pos.offset(0, 2, 0)));
        } catch (Exception e) {
    ...
    private void recoverWaterNow(EntityMaid maid) {
        ...
        try {
            ...
            BlockState st = level.getBlockState(this.waterPos);
            if (water != null && st.getBlock() == water) {
                level.setBlock(this.waterPos,
                        air != null ? air.defaultBlockState() : net.minecraft.world.level.block.Blocks.AIR.defaultBlockState(), 3);
            }
        } catch (Exception ignored) {
        }
        this.waterPos = null;
    ...
    private void tickRecoverWater(EntityMaid maid) {
        ...
        try {
            if (maid.level().getGameTime() - this.waterPlacedTick < 60) {
                return;
            }
            net.minecraft.server.level.ServerLevel level = (net.minecraft.server.level.ServerLevel) maid.level();
            // v1.5.240：原版静态引用（不依赖 Forge 注册表 getValue）
    ...
    private void antiSuffocate(EntityMaid maid) {
        ...
        net.minecraft.world.phys.AABB box = maid.getBoundingBox();
        double cy = box.minY + (box.maxY - box.minY) * 0.5;
        BlockPos mid = new BlockPos((int) Math.floor(maid.getX()),
                (int) Math.floor(cy), (int) Math.floor(maid.getZ()));
    ...
    private void giveUpMovement(EntityMaid maid) {
        if (maid.isInLava() && !hasFireResist(maid)
                && !(com.maidsmart.config.MaidSmartConfig.COMBAT_FIRE_PROTECT_BAUBLE.get()
                        && hasFireProtectBauble(maid))) {
            BlockPos edge = this.findLavaEdge(maid);
            if (edge != null) {
                maid.getNavigation().moveTo(edge.getX() + 0.5,
                        edge.getY(), edge.getZ() + 0.5, 1.2f);
            }
            return;
        }
        if (maid.isOnFire() && !hasFireResist(maid)
                && !(com.maidsmart.config.MaidSmartConfig.COMBAT_FIRE_PROTECT_BAUBLE.get()
                        && hasFireProtectBauble(maid))) {
            LivingEntity owner = maid.getOwner();
            if (owner != null && owner.isAlive()) {
                double odx = owner.getX() - maid.getX();
                double odz = owner.getZ() - maid.getZ();
                if (odx * odx + odz * odz > 4.0) { // 已贴身边就不用走，让主人收
                    maid.getNavigation().moveTo(owner.getX(), owner.getY(), owner.getZ(), 1.0f);
                }
            }
            return;
        }
        if (maid.isInWater() && maid.getAirSupply() < 60) {
            ...
            maid.setDeltaMovement(maid.getDeltaMovement().add(0.0, 0.28, 0.0));
            BlockPos air = this.findAirSpot(maid);
            if (air != null) {
                maid.getNavigation().moveTo(air.getX() + 0.5, air.getY(), air.getZ() + 0.5, 1.1f);
            } else {
                BlockPos shore = this.findShore(maid);
                if (shore != null) {
                    maid.getNavigation().moveTo(shore.getX() + 0.5, shore.getY(), shore.getZ() + 0.5, 1.1f);
                }
            }
            return;
        }
    ...
    private void environmentalEscape(EntityMaid maid) {
        ...
        //（v1.5.137：改用 isInLava = isInLava——旧 isInWaterOrRain 是 isInWaterOrLava，
        //  站水里也会误进岩浆逃生）
        if (maid.isInLava()) {
            ...
            // 流体（isInLava 对浅接触也为 true）就播报+垫高，明明走出去更优，
            ...
            if (safe != null && this.walkOutTimer <= 30 && this.hpRatio(maid) >= 0.4f) {
                maid.getNavigation().moveTo(safe.getX() + 0.5,
                        safe.getY(), safe.getZ() + 0.5, 1.2f);
                return;
            }
            ...
            if (!this.announcedEnv) {
                ...
                try {
                    ...
                    for (int i = 0; i < inv.getSlots(); i++) {
                        ...
                        if (s.getItem() == net.minecraft.core.registries.BuiltInRegistries.ITEM.get(
                        ...
                        if (com.maidsmart.tool.MaidBuildBlockFilter.isUsableBuildBlock(
                                net.minecraft.world.level.block.Block.byItem(s.getItem()),
                                maid.level(), maid.blockPosition())) { // v1.1.0 实测七：统一过滤
                            buildBlocks += s.getCount();
                        }
                    }
                    LOGGER.info("lava escape resources: maid={} dim={} bucket={} blocks={} fireResistPot={} pearl={} hp={}%",
                            maid.getDisplayName() != null ? maid.getDisplayName().getString() : "?",
                            maid.level().dimension().location().getPath(),
            ...
            if (!this.resourceUsed) {
                ...
                this.envGiveUpTick = maid.level().getGameTime();
            }
        ...
        if (maid.isOnFire()) {
            // v1.5.232：抗火随时断开——火不伤人，等自然灭即可（isOnFire 仍 true）
            if (hasFireResist(maid)) {
                return;
            }
            ...
            if (!isNether(maid)) {
                ...
                if (water != null) {
                    ...
                    double wx = water.getX() + 0.5;
                    double wz = water.getZ() + 0.5;
                    double dx = wx - maid.getX();
                    ...
                    if (distSq < 6.25) {
                        ...
                        maid.fallDistance = 0.0f; // 清摔落距离
                    } else {
                        maid.getNavigation().moveTo(wx, water.getY(), wz, 1.15f);
                    }
                    return;
                }
            }
            ...
            if (!this.resourceUsed && this.hpRatio(maid) < 0.6f) {
                ...
                try {
                    String hand = "?";
                    net.minecraft.resources.ResourceLocation hk = net.minecraft.core.registries.BuiltInRegistries.ITEM
                            .getKey(maid.getMainHandItem().getItem());
                    if (hk != null) {
                        hand = hk.toString();
                    }
                    net.minecraft.world.item.Item wb = net.minecraft.core.registries.BuiltInRegistries.ITEM.get(
        ...
        //（v1.5.137：isInWater = isInWater，旧 isInLava 实为 isInLava）
        ...
        if (maid.isInWater() && maid.getAirSupply() < 60) {
            ...
            maid.setDeltaMovement(maid.getDeltaMovement().add(0.0, 0.28, 0.0)); // 上浮辅助
        ...
        BlockPos air = this.findAirSpot(maid);
        if (air != null) {
            maid.getNavigation().moveTo(air.getX() + 0.5, air.getY(), air.getZ() + 0.5, 1.0f);
        } else {
            this.antiSuffocate(maid);
        }
        ...
        if (this.cachedNearLava != null && !hasFireResist(maid)) {
            ...
            if (this.cachedFleeSpot != null) {
                maid.getNavigation().moveTo(this.cachedFleeSpot.getX() + 0.5,
                        this.cachedFleeSpot.getY(), this.cachedFleeSpot.getZ() + 0.5, 1.2f);
            }
            return;
        }
    }
     ...
     *  v1.5.250【重大修复】：OVERWORLD 是 OVERWORLD（"overworld"），NETHER 是
     *  NETHER（"the_nether"）——旧代码拿 OVERWORLD 当 NETHER 比，主世界女仆被
     *  误判成"地狱"→ 水桶步/找水坑永远跳过 → "包里明明有水桶却不用水"（新存档
     *  不喜欢放水、着火不跳水坑的根因）。改回 NETHER。 */
    private static boolean isNether(EntityMaid maid) {
        return maid.level().dimension() == net.minecraft.world.level.Level.NETHER;
    }
    ...
    private boolean placeWaterOnLava(EntityMaid maid) {
        try {
            ...
            BlockPos target = lava.offset(0, 1, 0); // 最近岩浆格上方一格
            ...
            // v1.5.240：原版静态引用（不依赖 Forge 注册表 getValue）
            net.minecraft.world.level.block.Block water = net.minecraft.world.level.block.Blocks.WATER;
            level.setBlock(target, water.defaultBlockState(), 3);
    ...
    private boolean pearlEscapeFromLava(EntityMaid maid) {
        ...
        IItemHandler inv = maid.getMaidInv();
        net.minecraft.world.item.Item pearlItem = net.minecraft.core.registries.BuiltInRegistries.ITEM
                .get(ResourceLocation.parse("minecraft:ender_pearl"));
        int slot = -1;
        for (int i = 0; i < inv.getSlots(); i++) {
            ItemStack s = inv.getStackInSlot(i);
            if (!s.isEmpty() && pearlItem != null && s.getItem() == pearlItem) {
                slot = i;
                break;
            }
        }
        ...
        BlockPos land = this.findBlockBy(maid, 12, (level, p) -> {
            if (isLavaBlock(level.getBlockState(p).getBlock())) {
                return false; // 落点不能是岩浆
            }
            BlockPos below = p.offset(0, -1, 0);
            BlockState belowSt = level.getBlockState(below);
            if (isLavaBlock(belowSt.getBlock()) || !belowSt.isRedstoneConductor(level, below)) {
                return false; // 脚下岩浆/不实心
            }
            if (!level.getBlockState(p).isAir()
                    || !level.getBlockState(p.offset(0, 1, 0)).isAir()
                    || !level.getBlockState(p.offset(0, 2, 0)).isAir()) {
        ...
        if (!this.throwPearlTo(maid, level,
                land.getX() + 0.5, land.getY() + 0.5, land.getZ() + 0.5)) {
            // 投掷失败（异常兜底）→ 直接瞬移，保证逃生不失效
            maid.setPos(land.getX() + 0.5, land.getY() + 0.0, land.getZ() + 0.5);
            maid.fallDistance = 0.0f; // 珍珠瞬移落地不继承摔落距离
        }
    ...
    private boolean throwPearlTo(EntityMaid maid, net.minecraft.server.level.ServerLevel level,
        ...
        try {
            ...
            pearl.setPos(maid.getX(), maid.getEyeY() + 1.0, maid.getZ());
            double dx = tx - maid.getX();
            double dy = ty - (maid.getEyeY() + 1.0);
            double dz = tz - maid.getZ();
            ...
            pearl.setDeltaMovement(new net.minecraft.world.phys.Vec3(vx, vy, vz));
    ...
    private static boolean hasFireResist(EntityMaid maid) {
        for (net.minecraft.world.effect.MobEffectInstance eff : maid.getActiveEffects()) {
            net.minecraft.resources.ResourceLocation key = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT
                    .getKey(eff.getEffect().value());
            if (key != null && "minecraft:fire_resistance".equals(key.toString())) {
                if (eff.getDuration() >= 600) { // getDuration：剩余 ≥30 秒才算有效
                    return true;
                }
            }
        }
    ...
    public static boolean hasDeathSaveItem(EntityMaid maid) {
        ...
        try {
            ItemStack main = maid.getMainHandItem();
            if (!main.isEmpty() && main.getItem() == net.minecraft.world.item.Items.TOTEM_OF_UNDYING) {
                return true;
            }
            ItemStack off = maid.getOffhandItem();
            if (!off.isEmpty() && off.getItem() == net.minecraft.world.item.Items.TOTEM_OF_UNDYING) {
                return true;
            }
            net.neoforged.neoforge.items.IItemHandler inv = maid.getMaidInv();
            for (int i = 0; i < inv.getSlots(); i++) {
    ...
    private boolean throwFireResistPotion(EntityMaid maid) {
        try {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ...
                cloud.setPotionContents(new net.minecraft.world.item.alchemy.PotionContents(com.maidsmart.brew.PotionCompat.holderOf(stack))); // setPotion
                cloud.setRadius(4.0f);   // setRadius：4 格溅射范围（旧版误写 setRadiusOnUse=setRadiusOnFall，半径从未变大）
                cloud.setWaitTime(10);     // setWaitTime：0.5 秒后开始生效（旧版 setDuration 恰是 setWaitTime，碰巧正确）
                cloud.setDuration(600);    // setDuration：30 秒（旧版 m_19714_ 实为 setColor，写法错误成暗色云）
                cloud.setOwner(maid);   // setOwner
    ...
    private boolean eatGoldenAppleForFire(EntityMaid maid) {
        try {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ...
                net.minecraft.world.item.Item item = stack.getItem();
                if (item == net.minecraft.world.item.Items.ENCHANTED_GOLDEN_APPLE) { // enchanted_golden_apple
                    slot = i;
                    enchanted = true;
                    break; // 附魔金苹果效果最强，直接选
                }
                if (item == net.minecraft.world.item.Items.GOLDEN_APPLE && slot < 0) { // golden_apple
                    slot = i;
                }
            }
            if (slot < 0) {
            ...
            // v1.5.231b：真实进食（eat）——走原版食物属性：附魔金苹果 =
    ...
    /** 给目标施加效果（注册表取效果，避开 SRG 字段名；addEffect=addEffect） */
    private static void applyEffectTo(net.minecraft.world.entity.LivingEntity target,
        ...
        try {
            net.minecraft.world.effect.MobEffect effect = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT
                    .get(ResourceLocation.parse(effectId));
            if (effect != null) {
                target.addEffect(new net.minecraft.world.effect.MobEffectInstance(
                            net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect), duration, amplifier));
            }
        } catch (Exception ignored) {
        }
    }
    ...
    private boolean lavaStepUp(EntityMaid maid) {
        BlockPos pos = maid.blockPosition();
        BlockState st = maid.level().getBlockState(pos);
        ...
        maid.level().setBlock(pos, block.defaultBlockState(), 3);
    ...
    private boolean extinguishSelf(EntityMaid maid) {
        ...
        for (int i = 0; i < inv.getSlots(); i++) {
            ItemStack s = inv.getStackInSlot(i);
            if (s.isEmpty() || s.getItem() != ext) {
                continue;
            }
            inv.extractItem(i, 1, false);
            maid.level().addFreshEntity(new com.github.tartaricacid.touhoulittlemaid.entity.item.EntityExtinguishingAgent(
                    maid.level(), maid.position()));
    ...
    private BlockPos findWater(EntityMaid maid) {
        return this.findBlockBy(maid, 12, (level, p) -> isWaterBlock(level.getBlockState(p).getBlock()));
    }
    ...
    private void avoidLavaMovement(EntityMaid maid) {
        if (this.cachedNearLava == null || hasFireResist(maid) || maid.isInLava()) {
            return;
        }
        net.minecraft.world.phys.Vec3 vel = maid.getDeltaMovement();
        double hSpeed = Math.sqrt(vel.x * vel.x + vel.z * vel.z);
        if (hSpeed < 0.05) {
            return; // 没在移动，不干预
        }
        double hx = vel.x / hSpeed;
        double hz = vel.z / hSpeed;
        net.minecraft.world.level.Level level = maid.level();
        for (int step = 1; step <= 3; step++) {
            BlockPos p = new BlockPos((int) Math.floor(maid.getX() + hx * step),
                    (int) Math.floor(maid.getEyeY()),
                    (int) Math.floor(maid.getZ() + hz * step));
            if (!level.isLoaded(p)) {
                return; // 出了已加载区块就不预判
            }
            if (isLavaBlock(level.getBlockState(p).getBlock())) {
                ...
                if (this.cachedFleeSpot != null) {
                    maid.getNavigation().moveTo(this.cachedFleeSpot.getX() + 0.5,
                            this.cachedFleeSpot.getY(), this.cachedFleeSpot.getZ() + 0.5, 1.1f);
                }
                return;
            }
        }
        ...
        for (int dy = -2; dy <= 2; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
                for (int dz = -radius; dz <= radius; dz++) {
                    ...
                    if (isLavaBlock(maid.level().getBlockState(p).getBlock())) {
        ...
        for (int dy = -3; dy <= 3; dy++) {
            for (int dx = -8; dx <= 8; dx++) {
                for (int dz = -8; dz <= 8; dz++) {
                    ...
                    int dx2 = p.getX() - lava.getX();
                    int dz2 = p.getZ() - lava.getZ();
    ...
    private BlockPos findShore(EntityMaid maid) {
        return this.findBlockBy(maid, 10, (level, p) -> {
            if (isWaterBlock(level.getBlockState(p).getBlock())) {
                return false;
            }
            BlockPos below = p.offset(0, -1, 0);
    ...
    private BlockPos findAirSpot(EntityMaid maid) {
        return this.findBlockBy(maid, 4, (level, p) -> level.getBlockState(p).isAir()
                && level.getBlockState(p.offset(0, 1, 0)).isAir());
    }
    ...
    private boolean buildUp(EntityMaid maid) {
        ...
        BlockPos place;
        boolean posAir = maid.level().getBlockState(pos).isAir();
        if (posAir) {
            place = pos; // 悬空：垫所在格，站上去升高
        } else {
            place = pos.offset(0, 1, 0); // 平地：垫身体格顶起
        }
        ...
        double headY = maid.getBoundingBox().max(net.minecraft.core.Direction.Axis.Y);
        ...
        this.walkOnY = place.getY() + 1.0;
        this.walkOnTicks = 12;
        net.minecraft.world.phys.Vec3 cur = maid.getDeltaMovement();
        maid.setDeltaMovement(new net.minecraft.world.phys.Vec3(0.0,
                Math.max(cur.y, 0.42), 0.0));
        return true;
    }
    ...
    private void triggerPillarBurst(EntityMaid maid) {
        ...
        if (this.pillarBaseY < 0) {
            this.pillarBaseY = maid.blockPosition().getY();
        }
        if (!this.buildUp(maid)) {
    ...
    private void pushBackNearby(EntityMaid maid) {
        List<Mob> nearby = maid.level().getEntitiesOfClass(Mob.class,
                maid.getBoundingBox().inflate(5.0),
                m -> m instanceof LivingEntity && com.maidsmart.dialogue.PerceptionManager.isThreat((LivingEntity) m, maid));
        for (Mob m : nearby) {
            double dx = m.getX() - maid.getX();
            double dz = m.getZ() - maid.getZ();
            double len = Math.sqrt(dx * dx + dz * dz);
            if (len < 0.01) {
                continue; // 完全重叠：跳过
            }
            m.knockback(0.5, dx / len, dz / len); // 小力度击退（无伤害）
        }
    }
    ...
    private boolean bridgeStep(EntityMaid maid, LivingEntity threat) {
        ...
        int y = maid.blockPosition().getY();
        int tx1 = (int) Math.floor(maid.getX() + dx / len);
        int tz1 = (int) Math.floor(maid.getZ() + dz / len);
        int tx2 = (int) Math.floor(maid.getX() + dx / len * 2.0);
        int tz2 = (int) Math.floor(maid.getZ() + dz / len * 2.0);
        // 情况 A：前方 2 格是【实心满块】墙（v1.5.25：树叶/高草/水等非实心不算墙，
        // 防止"只喜欢原地平搭"乱垫浪费方块）→ 垫前方 1 格脚下，形成上升台阶
        BlockPos wallPos = new BlockPos(tx2, y, tz2);
        if (maid.level().getBlockState(wallPos).isRedstoneConductor(maid.level(), wallPos)) {
            BlockPos step = new BlockPos(tx1, y - 1, tz1);
            if (maid.level().getBlockState(step).isAir()
                    && maid.level().getBlockState(new BlockPos(tx1, y, tz1)).isAir()) {
        ...
        if (maid.level().getBlockState(gap).isAir()
                && maid.level().getBlockState(new BlockPos(tx1, y, tz1)).isAir()) {
    ...
    private void trySuffocateDuringBuild(EntityMaid maid, LivingEntity threat) {
        try {
            ...
            double bb = threat.getBoundingBox().max(net.minecraft.core.Direction.Axis.Y);
            double eyeY = threat.getEyeY(); // getEyeY
            if (eyeY - bb < 1.4) {
                return; // 低矮怪：眼睛格在自身格内，单方块封不住
            }
            BlockPos foot = threat.blockPosition();
            BlockPos eyeCap = foot.offset(0, 1, 0);
            ...
            maid.level().setBlock(eyeCap, block.defaultBlockState(), 3); // 头封块（窒息判定格）
    ...
    private boolean hasLocalEscape(EntityMaid maid) {
        try {
            ...
            net.minecraft.world.item.Item pearl = net.minecraft.core.registries.BuiltInRegistries.ITEM
                    .get(ResourceLocation.parse("minecraft:ender_pearl"));
            IItemHandler inv = maid.getMaidInv();
            for (int i = 0; i < inv.getSlots(); i++) {
                ItemStack s = inv.getStackInSlot(i);
                if (!s.isEmpty() && pearl != null && s.getItem() == pearl) {
                    return true;
                }
            }
        } catch (Exception ignored) {
    ...
    private boolean standingOnOwnTower(EntityMaid maid) {
        try {
            BlockPos feet = maid.blockPosition();
            return PlacedBlockTracker.trackedBlockId(maid.level(), feet.offset(0, -1, 0)) != null
                    || PlacedBlockTracker.trackedBlockId(maid.level(), feet.offset(0, -2, 0)) != null;
        } catch (Throwable ignored) {
            return false;
        }
    }
     ...
     * （= TLM 自己的 {@code DefaultMaidHealSelfMeal.isHealMeal} 口径：有食物属性组件且不在
    ...
    private boolean pearlEscape(EntityMaid maid, LivingEntity threat) {
        ...
        int tx = (int) Math.floor(maid.getX() + dx / len * 16.0);
        int tz = (int) Math.floor(maid.getZ() + dz / len * 16.0);
        net.minecraft.server.level.ServerLevel level = (net.minecraft.server.level.ServerLevel) maid.level();
        // 找该列的地面（从当前高度向下找到第一个实心格）
        int ty = maid.blockPosition().getY();
        while (ty > 0 && level.getBlockState(new BlockPos(tx, ty, tz)).isAir()) {
            ty--;
        }
        ...
        if (!level.getBlockState(belowPos).isAir() // 脚下是实心（非空）
                || !level.getBlockState(standPos).isAir() // 站立格是空气
                || !level.getBlockState(standPos.offset(0, 1, 0)).isAir() // 头顶 1 格空气
                || !level.getBlockState(standPos.offset(0, 2, 0)).isAir()) { // 头顶 2 格空气
            return false; // 落点会被活埋/卡墙 → 不扔（治旧版 stuck）
        }
        inv.extractItem(slot, 1, false);
        // v1.5.237：投掷原版末影珍珠（紫色轨迹 + 落地传送特效），不再直接瞬移
        if (!this.throwPearlTo(maid, level, tx + 0.5, standPos.getY() + 0.5, tz + 0.5)) {
            // 投掷失败（异常兜底）→ 直接瞬移
            maid.setPos(tx + 0.5, standPos.getY() + 0.0, tz + 0.5);
    ...
    private void giveBottle(EntityMaid maid) {
        net.minecraft.world.item.Item bottle = net.minecraft.core.registries.BuiltInRegistries.ITEM
                .get(ResourceLocation.parse("minecraft:glass_bottle"));
        if (bottle != null) {
        ...
        try {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ...
                for (net.minecraft.world.effect.MobEffectInstance e :
                    ...
                    maid.addEffect(new net.minecraft.world.effect.MobEffectInstance(e));
                    maxDur = Math.max(maxDur, e.getDuration());
                }
                maid.swing(net.minecraft.world.InteractionHand.MAIN_HAND);
                this.resourceUsed = true;
                // v1.5.252g7：按药水种类记 CD（= 该药水最长时长；瞬间效果短 CD）
                this.markPotionUsed(potionKey(stack), maid.level().getGameTime(), maxDur > 0 ? maxDur : 40);
        ...
        try {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ...
                net.minecraft.world.entity.AreaEffectCloud cloud =
                        new net.minecraft.world.entity.AreaEffectCloud(level,
                                maid.getX(), maid.getEyeY(), maid.getZ());
                cloud.setPotionContents(new net.minecraft.world.item.alchemy.PotionContents(net.minecraft.core.registries.BuiltInRegistries.POTION.wrapAsHolder(potion)));   // setPotion
                cloud.setRadius(4.0f);     // setRadius（旧版误写 setRadiusOnUse=setRadiusOnFall）
                cloud.setWaitTime(10);       // setWaitTime（旧版 setDuration 恰是 setWaitTime，碰巧正确）
                cloud.setDuration(600);      // setDuration：30 秒云（旧版 m_19714_ 实为 setColor，误用）
                cloud.setOwner(maid);     // setOwner
                level.addFreshEntity(cloud);
                inv.extractItem(i, 1, false);
                maid.swing(net.minecraft.world.InteractionHand.MAIN_HAND);
                ...
                LOGGER.info("self drink: potion={} hp={}%",
                        ...
                        String.format("%.0f", this.hpRatio(maid) * 100.0f));
                int maxDur = 0;
                for (net.minecraft.world.effect.MobEffectInstance e : potion.getEffects()) {
                    maxDur = Math.max(maxDur, e.getDuration());
                }
                // v1.5.252g7：按药水种类记 CD（= 该药水最长时长；瞬间效果短 CD）
                this.markPotionUsed(potionKey(stack), level.getGameTime(), maxDur > 0 ? maxDur : 40);
                return Math.max(40, Math.min(maxDur > 0 ? maxDur : 40, 12000));
            }
        } catch (Exception ignored) {
        }
    ...
    potionEffectsOf(ItemStack stack) {
        ...
        try {
            net.minecraft.world.item.alchemy.Potion p =
                    com.maidsmart.brew.PotionCompat.of(stack);
            if (p != null) {
                out.addAll(p.getEffects());
            }
            out.addAll(com.maidsmart.brew.PotionCompat.customEffects(stack));
        } catch (Exception ignored) {
        }
        return out;
    }
    ...
    private static boolean potionHasEffect(net.minecraft.world.item.alchemy.Potion potion, String effectId) {
        try {
            for (net.minecraft.world.effect.MobEffectInstance e : potion.getEffects()) {
                net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> effect = e.getEffect();
                if (effect == null) {
                    continue;
                }
                net.minecraft.resources.ResourceLocation key = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.getKey(effect.value());
    ...
    private boolean drinkHoneyForPoison(EntityMaid maid) {
        try {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ItemStack stack = inv.getStackInSlot(i);
                if (stack.isEmpty() || stack.getItem() != net.minecraft.world.item.Items.HONEY_BOTTLE) {
                    continue; // honey_bottle
                }
                inv.extractItem(i, 1, false);
                this.giveBottle(maid); // 喝完留玻璃瓶（原版一致）
                maid.removeEffect(net.minecraft.world.effect.MobEffects.POISON); // removeEffect(POISON)
                maid.heal(3.0f);    // 回 1.5 心（近似原版饥饿→饱和恢复）
                maid.swing(net.minecraft.world.InteractionHand.MAIN_HAND);
                this.resourceUsed = true;
                return true;
            }
    ...
    /**
     * v1.5.281：负面效果自清（会话内外常驻，见 tick 调用点）——
    ...
    private void tickCureNegativeEffects(EntityMaid maid) {
        ...
        if (this.potionReady("honey", nowTick)
                && maid.hasEffect(net.minecraft.world.effect.MobEffects.POISON) // hasEffect(POISON)
    ...
    private static boolean hasOnlyNegativeEffects(EntityMaid maid) {
        ...
        for (net.minecraft.world.effect.MobEffectInstance ei : maid.getActiveEffects()) {
            net.minecraft.world.effect.MobEffectCategory cat = ei.getEffect().value().getCategory();
    ...
    private boolean drinkMilkBucket(EntityMaid maid) {
        try {
            IItemHandler inv = maid.getMaidInv();
            net.minecraft.world.item.Item milk = net.minecraft.core.registries.BuiltInRegistries.ITEM
                    .get(ResourceLocation.parse("minecraft:milk_bucket"));
            if (milk == null) {
                return false;
            }
            for (int i = 0; i < inv.getSlots(); i++) {
                ItemStack stack = inv.getStackInSlot(i);
                if (stack.isEmpty() || stack.getItem() != milk) {
                    continue;
                }
                inv.extractItem(i, 1, false);
    ...
    private void giveBucket(EntityMaid maid) {
        net.minecraft.world.item.Item bucket = net.minecraft.core.registries.BuiltInRegistries.ITEM
                .get(ResourceLocation.parse("minecraft:bucket"));
        if (bucket != null) {
    ...
    private boolean useGoldenApple(EntityMaid maid) {
        try {
            ...
            for (int i = 0; i < inv.getSlots(); i++) {
                ...
                if (item == net.minecraft.world.item.Items.GOLDEN_APPLE && bestSlot < 0) { // golden_apple
                    bestSlot = i;
                }
            }
            if (bestSlot < 0) {
    ...
    /** 施加原版金苹果效果（数值与原版食物一致；addEffect=addEffect） */
    private static void applyGoldenAppleEffects(net.minecraft.world.entity.LivingEntity target, boolean enchanted) {
        try {
            net.minecraft.world.effect.MobEffect absorption = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT
                    .get(ResourceLocation.parse("minecraft:absorption"));
            net.minecraft.world.effect.MobEffect regen = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT
                    .get(ResourceLocation.parse("minecraft:regeneration"));
            if (absorption != null) {
                target.addEffect(new net.minecraft.world.effect.MobEffectInstance(
                            net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.wrapAsHolder(absorption), 2400, enchanted ? 3 : 0));
            }
            if (regen != null) {
                target.addEffect(new net.minecraft.world.effect.MobEffectInstance(
                            net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.wrapAsHolder(regen), enchanted ? 600 : 100, enchanted ? 4 : 1));
            }
            if (enchanted) {
                net.minecraft.world.effect.MobEffect res = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT
                        .get(ResourceLocation.parse("minecraft:damage_resistance"));
                net.minecraft.world.effect.MobEffect fire = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT
                        .get(ResourceLocation.parse("minecraft:fire_resistance"));
                if (res != null) {
                    target.addEffect(new net.minecraft.world.effect.MobEffectInstance(
                            net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.wrapAsHolder(res), 6000, 0));
                }
                if (fire != null) {
                    target.addEffect(new net.minecraft.world.effect.MobEffectInstance(
                            net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.wrapAsHolder(fire), 6000, 0));
                }
            }
        } catch (Exception ignored) {
        }
    ...
    private int drinkBeneficialPass(EntityMaid maid, IItemHandler inv, long nowTick, boolean drinkOnly) {
        for (int i = 0; i < inv.getSlots(); i++) {
            ...
            for (net.minecraft.world.effect.MobEffectInstance ei : effects) {
                net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> effect = ei.getEffect();
                if (effect == null) {
                    continue;
                }
                ...
                net.minecraft.resources.ResourceLocation eid = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.getKey(effect.value());
                ...
                // v1.5.25d 修复：isInstantenous() 基类恒返回 false（不是 isBeneficial）——
                // 旧判断永远 false → 增益药水全被跳过（"仍不喝再生/迅捷"的根因）。
                // 改用 category 判断：getCategory() == MobEffectCategory.BENEFICIAL
                boolean isBeneficial = effect.value().getCategory() == net.minecraft.world.effect.MobEffectCategory.BENEFICIAL;
            ...
            for (net.minecraft.world.effect.MobEffectInstance ei : effects) {
                ...
                maid.addEffect(new net.minecraft.world.effect.MobEffectInstance(ei));
                maxDur = Math.max(maxDur, ei.getDuration());
            }
    ...
    private void spawnAlert(EntityMaid maid) {
        ...
        try {
            net.minecraft.core.particles.SimpleParticleType angry =
                    (net.minecraft.core.particles.SimpleParticleType) net.minecraft.core.registries.BuiltInRegistries.PARTICLE_TYPE
                            .get(ResourceLocation.parse("minecraft:angry_villager"));
            if (angry != null && maid.level() instanceof ServerLevel level) {
                level.sendParticles(angry, maid.getX(), maid.getY() + 2.2, maid.getZ(),
                        4, 0.2, 0.3, 0.2, 0.0);
            }
        } catch (Exception ignored) {
        }
    ...
    private Block takeBuildBlock(EntityMaid maid) {
        ...
        net.minecraft.world.item.Item item = com.maidsmart.tool.MaidBuildBlockFilter
                .takeBuildBlock(maid.getMaidInv(), maid.level(), maid.blockPosition());
        if (item == null) {
            return null;
        }
        return net.minecraft.core.registries.BuiltInRegistries.BLOCK.get(net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(item));
    }
    ...
    private boolean isSafeBuildBlock(EntityMaid maid, Block block) {
        return com.maidsmart.tool.MaidBuildBlockFilter.isUsableBuildBlock(
                block, maid.level(), maid.blockPosition());
    }
}
