...
public final class MaidFlightKit {
    ...
    private static ItemStack resolveWeapon(EntityMaid maid, StackFilter filter) {
        ...
        try {
            if (filter.test(maid.getMainHandItem())) {
    ...
    public static ItemStack findRiptide(EntityMaid maid) {
        ...
        try {
            if (isRiptide(maid.getMainHandItem())) {
                return maid.getMainHandItem(); // 主手
            }
            if (isRiptide(maid.getOffhandItem())) {
                return maid.getOffhandItem(); // 副手
            }
        } catch (Throwable ignored) {
        }
        try {
    ...
    /**
     * v1.2.4 实测六百三十九【先看身上，再问额外容器】：**真要动手推一口时**调这个。
     ...
     * 两者都 extends {@code CuriosSlotRef}、都按 NeoForge 的 {@code Capabilities.ItemHandler}
     * 操作饰品栏里那件背包），而本方法走的是 TLM 自己的容器列表——所以**旅行者背包与精妙背包
     * 从第一天起就是同一条路**，不需要各写一份。
     */
     ...
     *  ——与 {@link #hasFan} / {@link #hasFirework} 同款（见 {@link #borrowedOffhand}）。
    ...
    public static boolean hasRiptide(EntityMaid maid) {
        ...
        if (!findRiptide(maid).isEmpty()) {
            return true; // 主手 → 副手 → 背包（实测六百四十 起三处都算）
        }
    ...
    public static boolean hasClimbSpell(EntityMaid maid) {
        ...
        try {
            ...
            Long cached = DASH_KIT_CACHE_TICK.get(maid.getUUID());
    ...
    public static String fuelDiagnostic(EntityMaid maid) {
        ...
        return "燃料判定: 主手=" + describeStack(maid.getMainHandItem())
                ...
                + " 身上激流三叉戟=" + !findRiptide(maid).isEmpty()
                + " 额外容器激流三叉戟=" + extra
     ...
     * 而是它活着的那十几 tick 里**每 tick 都把骑手速度往"1.7×视线"上拉**——1.21.1
     * `FireworkRocketEntity.tick` 字节码与 1.20.1 同形（那一支只判
     * {@code rider.isFallFlying()}，也就是我们置的滑翔位）：
     * <pre>
     *       Vec3 look = rider.getLookAngle();
     *       rider.setDeltaMovement(vel.add(
     *           look.x * 0.1 + (look.x * 1.5 - vel.x) * 0.5, …y…, …z…));
     * </pre>
     * 即 `v = v * 0.5 + look * 0.85`（不动点 = 1.7 倍视线方向）。所以"进到主人身边就解除"
     * 必须把**还挂在背上的那枚**收掉（{@code discard()}），否则下一 tick 它又把速度补回来，
     * 清零等于白清。
     *
