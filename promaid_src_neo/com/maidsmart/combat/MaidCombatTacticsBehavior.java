 ...
 * 在最末尾，导航天然最后覆盖。
 *
 * 移动接管方式（与自保逃跑同款）：每 tick 清 WALK_TARGET + 直连导航（moveTo）。
...
public class MaidCombatTacticsBehavior extends Behavior<EntityMaid> {
    ...
    public static boolean shouldUseShield(EntityMaid maid) {
        ...
        if (maid.getMainHandItem().getItem() instanceof ProjectileWeaponItem
        ...
        return maid.getBrain().getMemory(MemoryModuleType.ATTACK_COOLING_DOWN).isPresent();
    }
    ...
    protected void start(ServerLevel level, EntityMaid maid, long gameTime) {
        this.orbitSide = maid.getRandom().nextFloat() < 0.5 ? -1 : 1;
        this.orbitAngle = maid.getRandom().nextInt(628) / 100.0; // 0~6.28
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        // v1.5.137：旧 isSuppressingBounce() 实为 isSneaking（字节码实证 getFlag(1)），
    ...
    private void meleeTick(EntityMaid maid, LivingEntity target) {
        ...
        // "战斗 + 非自保"：本行为在自保移动类保命动作时已让位（tick 开头
        ...
        if (dist > MELEE_MELEE_RANGE) {
            ...
            if (this.jumpCooldown <= 0 && dist > 1.8) {
                ...
                if (axeMode) {
                    ...
                    tryJump = crowd ? false
                            : maid.getRandom().nextFloat() < SOLO_JUMP_CHANCE;
                }
                if (tryJump) {
                    ...
                    this.dashAttack = dist > JUMP_DASH_DIST
                            || (dashReady && maid.getRandom().nextFloat() < 0.3);
                    if (this.dashAttack) {
                        ...
                        net.minecraft.world.phys.Vec3 mv = maid.getDeltaMovement();
                        maid.setDeltaMovement(new net.minecraft.world.phys.Vec3(
                                mv.x(), JUMP_VELOCITY, mv.z()));
                    }
                    this.jumpAir = true; // 标记跳劈空中状态（暴击挥砍检测用）
                    this.critDone = false;
                    // v1.5.155：记录起跳时刻（挥砍延迟——先跳、隔一会再出刀）
                    this.jumpStartTime = maid.level().getGameTime();
            ...
            if (this.orbitSwapTick-- <= 0) {
                this.orbitSwapTick = 12 + maid.getRandom().nextInt(9);
    ...
    private int nearbyEnemies(LivingEntity target, double radius) {
        try {
            return target.level().getEntitiesOfClass(net.minecraft.world.entity.monster.Monster.class,
                    target.getBoundingBox().inflate(radius), m -> m.isAlive()).size();
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    private void tickJumpCrit(ServerLevel level, EntityMaid maid, LivingEntity target) {
        ...
        // 下落中段出刀）
        long elapsed = level.getGameTime() - this.jumpStartTime;
        int swingDelay = this.dashAttack ? 11 : 7;
        if (elapsed < swingDelay) {
            return;
        }
        if (!this.critDone && target.isAlive()
                && maid.distanceTo(target) <= MELEE_MELEE_RANGE + 0.5) {
    ...
    private boolean swingCritHit(ServerLevel level, EntityMaid maid, LivingEntity target) {
        ...
        for (net.minecraft.core.Holder<net.minecraft.world.item.enchantment.Enchantment> h
                : weapon.getEnchantments().keySet()) {
            org.apache.commons.lang3.mutable.MutableFloat bonus =
                    new org.apache.commons.lang3.mutable.MutableFloat(0.0f);
            h.value().modifyDamage(level, weapon.getEnchantments().getLevel(h), weapon,
                    target, target.damageSources().mobAttack(maid), bonus);
            ench += bonus.floatValue();
        }
        float dmg = base * 1.5F + ench; // 玩家暴击：攻击力 × 1.5 + 附魔加成
        boolean hit = target.hurt(maid.damageSources().mobAttack(maid), dmg);
        if (!hit) {
            return false;
        }
        if (this.dashAttack) {
            ...
            double len = Math.sqrt(dx * dx + dz * dz);
            if (len > 0.01) {
                target.knockback(1.5, dx / len, dz / len);
            }
            ...
            LOGGER.info("jump dash crit: maid={} target={} dmg={}",
                    maid.getDisplayName() != null ? maid.getDisplayName().getString() : maid.getUUID(),
                    target.getDisplayName() != null ? target.getDisplayName().getString() : target.getType().toString(),
        ...
        int fireAspect = net.minecraft.world.item.enchantment.EnchantmentHelper.getItemEnchantmentLevel(
        ...
        if (fireAspect > 0 && !target.isOnFire()) {
            target.setRemainingFireTicks(fireAspect * 4 * 20);
        }
        net.minecraft.world.item.enchantment.EnchantmentHelper.doPostAttackEffects(
                level, target, target.damageSources().mobAttack(maid));
        return true;
    }
        ...
        try {
            net.minecraft.core.particles.SimpleParticleType crit =
                    (net.minecraft.core.particles.SimpleParticleType) net.minecraft.core.registries.BuiltInRegistries.PARTICLE_TYPE
                            .get(net.minecraft.resources.ResourceLocation.parse("minecraft:crit"));
            if (crit == null) {
                return;
            }
            double x = target.getX();
            double y = target.getY() + target.getBbHeight() * 0.6; // 身体中上部
            double z = target.getZ();
            level.sendParticles(crit, x, y, z, count, 0.3, 0.2, 0.3, 0.2);
    ...
    private void playCritSound(ServerLevel level, LivingEntity target) {
        try {
            ...
            level.playSound(null, target.blockPosition(), snd,
    ...
    private void rangedTick(EntityMaid maid, LivingEntity target) {
        ...
        int maxRange = main.getItem() instanceof ProjectileWeaponItem pw
                ? pw.getDefaultProjectileRange()
        ...
        if (++this.rangedDiagTick >= 40) {
            ...
            LOGGER.info("ranged kite: maid={} task={} main={} dist={} ideal={}",
                    ...
                    main.isEmpty() ? "empty"
                            : net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(main.getItem()),
        ...
        if (dist < ideal * 0.75) {
            ...
            if (this.orbitSwapTick-- <= 0) {
                this.orbitSwapTick = 15 + maid.getRandom().nextInt(11);
                this.orbitSide = -this.orbitSide;
            }
    ...
    private void meleeCounterAttack(EntityMaid maid, LivingEntity target) {
        try {
            ...
            level.levelEvent(2004, target.blockPosition(), 0);
            ...
            for (LivingEntity victim : level.getEntitiesOfClass(LivingEntity.class,
                ...
                double dz = victim.getZ() - maid.getZ();
                double len = Math.max(0.01, Math.sqrt(dx * dx + dz * dz));
                victim.knockback(0.5, dx / len, dz / len);
            }
        } catch (Exception ignored) {
        }
    }

    /** 物品攻击力属性值（ItemStack.m_41638_ 属性修饰表 → m_22182_ 基值+lambda$getSyncableAttributes$0？
    ...
    private static float weaponAttack(ItemStack stack) {
        try {
            ...
            float total = -1.0f;
            for (net.minecraft.world.item.component.ItemAttributeModifiers.Entry en : mods) {
                if (!en.attribute().is(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE)) {
                    continue;
                }
                if (total < 0) {
                    total = 0;
                }
                total += (float) en.modifier().amount();
            }
            return total;
        } catch (Exception e) {
            return -1.0f;
    ...
    private void navigateTo(EntityMaid maid, LivingEntity target, float speed) {
        maid.getNavigation().moveTo(target.getX(), target.getY(), target.getZ(), speed);
    }
    ...
    private void navigateAway(EntityMaid maid, LivingEntity target, double dist, float speed) {
        ...
        double len = Math.max(0.01, Math.sqrt(dx * dx + dz * dz));
        double tx = maid.getX() + dx / len * dist;
        double tz = maid.getZ() + dz / len * dist;
        maid.getNavigation().moveTo(tx, this.groundY(maid, tx, tz), tz, speed);
    }
    ...
    private void navigateArc(EntityMaid maid, LivingEntity target, double side, float speed) {
        ...
        double dz = target.getZ() - maid.getZ();
        double len = Math.max(0.01, Math.sqrt(dx * dx + dz * dz));
        double px = target.getX() + (-dz / len) * side;
        double pz = target.getZ() + (dx / len) * side;
        maid.getNavigation().moveTo(px, this.groundY(maid, px, pz), pz, speed);
    }
    ...
    private void navigateOrbit(EntityMaid maid, LivingEntity target, double r, float speed) {
        ...
        double px = target.getX() + Math.cos(this.orbitAngle) * r;
        double pz = target.getZ() + Math.sin(this.orbitAngle) * r;
        maid.getNavigation().moveTo(px, this.groundY(maid, px, pz), pz, speed);
    }

    /** 目标点地面高度：从女仆当前 y 向下找第一个非空气格（防导航目标悬空） */
    private double groundY(EntityMaid maid, double x, double z) {
        int y = (int) Math.floor(maid.getY());
        for (int i = 0; i < 6; i++) {
            if (!maid.level().getBlockState(new net.minecraft.core.BlockPos(
                    (int) Math.floor(x), y - 1 - i, (int) Math.floor(z))).isAir()) {
                return y - i;
            }
        }
        return maid.getY();
