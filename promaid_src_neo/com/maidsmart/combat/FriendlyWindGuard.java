...
public final class FriendlyWindGuard {
     ...
     * `attribute_to_user` 缺省 false（`data/minecraft/enchantment/wind_burst.json` 实证：既无
     * attribute_to_user 也无 damage_type）——`ExplodeEffect.apply` 于是传 `entity = null`，
     * 爆炸的直系/间系来源**都是 null**，前两条全部落空 → 击退照推（**只震不伤**，伤害那条
     * 路本来就没有）。反馈的"主人被女仆重锤波及"就是这一条：女仆俯冲砸怪时风爆在**她身上**
     * 炸开，旁边的主人被 3.5 格半径的冲击波掀飞。
     *
    ...
    public static EntityMaid maidOfExplosion(Explosion ex) {
        ...
        try {
            ...
            m = maidOf(ex.getIndirectSourceEntity());
            if (m != null) {
                return m;
            }
