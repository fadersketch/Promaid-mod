 ...
 *
 * 1.【攻击路径】燃烧的女仆对主人造成任何伤害（LivingIncomingDamageEvent，最上游可取消）
 ...
 *   贴身的燃烧女仆，直接给主人灭火（clearFire = clearFire，javap 核实内部
 *   调 setRemainingFireTicks(0)）。
 *   主人自己站在火/岩浆里（或浸在水中）则不干预——那是玩家自己的火，
 *   贸然灭火就成了作弊外挂。
 *
 * 火状态 SRG（javap 反编译核实）：getRemainingFireTicks = getRemainingFireTicks，
 * clearFire = clearFire，setRemainingFireTicks = setRemainingFireTicks。
...
public class MaidFireGuard {
    ...
    @SubscribeEvent
    public void onOwnerAttackedByMaid(LivingIncomingDamageEvent event) {
        ...
        if (src == null || !(src.getEntity() instanceof EntityMaid maid)) {
            return;
        }
        if (maid.getRemainingFireTicks() <= 0) {
            return; // 女仆没烧着，与本病无关
        }
...
if (++this.throttle < 10) {
        ...
        for (ServerLevel level : event.getServer().getAllLevels()) {
            for (net.minecraft.world.entity.Entity e : level.getAllEntities()) {
                ...
                if (!(maid.getOwner() instanceof ServerPlayer owner) || owner.isDeadOrDying()) {
                    continue;
                }
                if (owner.getRemainingFireTicks() <= 0) {
                    continue; // 主人没着火
                }
                // 贴身接触（≤2 格）
                if (maid.position().distanceTo(owner.position()) > 4.0) {
                    continue;
                }
                ...
                owner.clearFire(); // clearFire——火来自贴身的燃烧女仆，直接扑灭
            }
        }
    }
    ...
    private static boolean selfInFireOrFluid(ServerPlayer p) {
        ...
        net.minecraft.core.BlockPos base = p.blockPosition();
        for (net.minecraft.core.BlockPos pos : new net.minecraft.core.BlockPos[]{base, base.offset(0, 1, 0)}) {
            var st = lvl.getBlockState(pos);
            if (st.getBlock() instanceof net.minecraft.world.level.block.BaseFireBlock) {
                return true;
            }
            if (!st.getFluidState().isEmpty()) {
                return true; // 非空流体（isEmpty = FluidState.isEmpty，工程内已核实口径）
            }
        }
