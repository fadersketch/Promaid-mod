 ...
 *       两边都反编译核过）。</li>
 *   <li><b>管线</b>：原版自己的拴绳渲染类型（1.21.1 {@code RenderType.leash()} /
 *       1.20.1 {@code RenderType.m_110475_()}，{@code POSITION_COLOR_LIGHTMAP} + 自己的 shader
 *       + {@code NO_CULL}——正因如此原版那两趟反向绕序的带子才都能看见），不再是 LINES。</li>
 *   <li><b>几何</b>：沿绳子分 24 段（原版 {@code EntityRenderer.LEASH_RENDER_STEPS} / 1.20.1
 *       {@code MobRenderer.f_174302_}），每段写一对顶点：两趟 {@code TRIANGLE_STRIP} 围成一条
...
public final class GunnerTetherClient {
    ...
    public static void onRender(net.neoforged.neoforge.client.event.RenderLevelStageEvent event) {
        ...
        // 【实测六百七十六】按帧插值要用的 partialTick（1.21.1 的事件给的是 DeltaTracker）
    ...
    /**
     * 原版 {@code EntityRenderer.addVertexPair} 的逐字搬运（只改了颜色、加了拖曳弯曲）：
     * 第 {@code step} 段的一对顶点。
     *
    ...
    private static void addVertexPair(VertexConsumer vc, Matrix4f mat, float dx, float dy, float dz,
        ...
        int light = LightTexture.pack(lerpInt(f, blockA, blockB), lerpInt(f, skyA, skyB));
        ...
        vc.addVertex(mat, x - ox, y + h2, z + oz).setColor(r, g, bl, 1.0f).setLight(light);
        vc.addVertex(mat, x + ox, y + h1 - h2, z - oz).setColor(r, g, bl, 1.0f).setLight(light);
    }
    ...
    private static float[] dragFor(Entity maid) {
        ...
        float tx = clamp((float) (maid.xo - maid.getX()) * ROPE_DRAG_K, ROPE_DRAG_MAX);
        float tz = clamp((float) (maid.zo - maid.getZ()) * ROPE_DRAG_K, ROPE_DRAG_MAX);
    ...
    private static double lerp(double from, double to, float pt) {
        return Mth.lerp((double) pt, from, to);
    }
