...
/**
 * v1.2.0（客户端）：滑翔中强制鞘翅**完全展开**。
 *
 * 【为什么需要】原版 `ElytraModel.setupAnim` 是"按下落速度决定展开程度"的
 * （1.21.1 bytecode 实证）：`f = 1 - pow(-vy_normalized, 1.5)`，
 * 翅膀角度在折叠态（xRot 0.2618 / zRot -0.2618）与完全展开态
 * （xRot 0.34906584 / zRot -1.5707964）之间按 f 插值。
 * 女仆扑向目标时基本都在下降，f 会被拉低 → 看上去"没张开"。
