...
public final class EmotionKeysClient {
     ...
     *  Shift 修饰方案作废——Screen.hasShiftDown 的 SRG 归属未做字节码实证，
...
Minecraft mc = Minecraft.getInstance();
            ...
            while (MAID_PAT != null && MAID_PAT.consumeClick()) {
                LOG.info("emotion key: PAT pressed, sending C2S");
                PacketDistributor.sendToServer(new EmotionNetworking.TriggerEmotionPacket(0));
            }
            while (MAID_HUG != null && MAID_HUG.consumeClick()) {
                LOG.info("emotion key: HUG pressed, sending C2S");
                PacketDistributor.sendToServer(new EmotionNetworking.TriggerEmotionPacket(1));
            }
        }
    }
}
