...
public final class EmotionPoseState {
    ...
    public static void start(EntityMaid maid, byte type) {
        ...
        POSES.put(maid.getUUID(), new long[]{type, maid.level().getGameTime() + duration});
    }

    /** 当前是否在指定姿势中（自动清理过期项） */
    public static boolean isActive(EntityMaid maid, byte type) {
        long[] pose = POSES.get(maid.getUUID());
        if (pose == null) {
            return false;
        }
        if (maid.level().getGameTime() > pose[1]) {
            POSES.remove(maid.getUUID());
            return false;
        }
        return pose[0] == type;
    }
