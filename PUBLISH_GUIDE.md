...
- **与「是，史蒂夫模型」(YSM) 共存**：不会崩——v1.2.0（实测五百四十八）修掉了导致**黑屏进不去游戏**的 `ClassCastException`；**两个图层在 YSM 模型上现在都绘制**：激流旋转特效（实测六百八十三，那一层不依赖骨骼）与**鞘翅**（实测六百八十四——旧版要查 Gecko 骨骼表定挂点、YSM 没有那套骨骼所以整层跳过，现在非 Gecko 渲染器改走**固定身体偏移**：肩线 +1.86 格，取自 28 个模型的统计值；YSM 调图层的空间与 TLM 的 `GeoReplacedEntityRenderer` 同构，与激流那层同一套坐标）。两树同步修复。
- **其他模组的鞘翅装备**（实测六百八十四）：通用判据 = **这件装备对玩家能不能滑翔**（`ItemStack.canElytraFly`，与原版滑翔的闸门同源）。能滑翔、且**本身不是护甲**的，女仆背上就画翅膀（贴图用原版 `elytra.png`），模组无需为本模组适配；**本身是护甲的那一类**（鞘翅胸甲）不叠翅膀（自带外观，叠了穿模，原版 `ElytraLayer` 也只服务原版鞘翅），该判定会写进日志（搜 `鞘翅渲染`）。
