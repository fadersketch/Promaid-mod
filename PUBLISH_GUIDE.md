...
- **与「是，史蒂夫模型」(YSM) 共存**：不会崩——v1.2.0（实测五百四十八）修掉了导致**黑屏进不去游戏**的 `ClassCastException`；**鞘翅在 YSM 模型上不绘制**（YSM 模型没有 Gecko 骨骼表，硬画只会把翅膀堆在原点，所以那一层在 YSM 渲染器下主动跳过），女仆的背部物品/鞘翅由 TLM 自己的 YSM 图层负责。**激流旋转特效从实测六百八十三 起在 YSM 模型上也绘制**（那一层不依赖任何骨骼，五百四十八 里"两个图层都不画"已撤）。两树同步修复。
